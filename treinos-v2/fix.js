...
PARTE('a aba coach e so da academia', function(){
  ...
  function etapasDaAcademia(s){
    try{
      ...
      var aq = r.aquecimento ? [{ id: 'acAq', t: 'Aquecimento', d: r.aquecimento, tags: [] }] : [];
      return aq.concat(r.itens.map(function(it, i){
        var tags = [];
  ...
  if(typeof window.renderDia === 'function'){
    ...
    window.renderDia = function(){
      ...
      try{
        ...
        /* O DIA DA ACADEMIA NO CARTAO: o mesmo bloco da aba Coach (onde,
           lista, situacao, botao certo). Saem a lista de caixinhas, o
           Concluir e o antigo botao do Hevy: marcar caixinha nao dizia
           nada — quem diz feito e o Hevy ou o botao de casa. O guarda
           da classe evita bloco repetido (renderDia e embrulhado mais
           de uma vez). */
        /* dia em que a academia e o treino PRINCIPAL (dia sem corrida no
           plano): o mesmo bloco, no cartao principal. Saem as caixinhas,
           a barra de etapas, o Concluir e o "Ver e enviar ao MOTRA";
           Mover dia fica. */
        if(el && s && daAcademia(s) && (s.data || sel) && typeof window.bqAcadCartao === 'function' &&
           !el.querySelector(':scope > .bqAcadCard')){
          [':scope > .prog', ':scope > .etapas', ':scope > .acts [data-act="motra"]',
           ':scope > .acts [data-act="concluir"]'].forEach(function(sq){
            el.querySelectorAll(sq).forEach(function(n){ if(n.parentNode) n.parentNode.removeChild(n) });
          });
          var blocoP = document.createElement('div');
          blocoP.className = 'bqAcadCard';
          blocoP.style.cssText = 'margin:0 14px 12px';
        ...
        if(ex && daAcademia(x) && x.data && typeof window.bqAcadCartao === 'function' &&
          ...
          ['.etapas', '.ptrack', '.plab', '[data-exok]', '[data-exmotra]'].forEach(function(sq){
            ex.querySelectorAll(sq).forEach(function(n){ if(n.parentNode) n.parentNode.removeChild(n) });
          });
          var bloco = document.createElement('div');
          bloco.className = 'bqAcadCard';
          bloco.style.cssText = 'margin:10px 0 4px';
          bloco.innerHTML = window.bqAcadCartao(x.data);
          var antesDe = ex.querySelector('.acts') || ex.querySelector('.btn-cancelar');
          if(antesDe) ex.insertBefore(bloco, antesDe); else ex.appendChild(bloco);
        }
...
PARTE('painel da academia', function(){
  ...
  function ehDaFit4less(nome){ return /fit\s*4\s*less/.test(semAcento(nome)) }

  /* tipo:
       'base'     — a rotina de sempre do Hevy (sem "Casa" nem "Fit4Less")
       'casa'     — so rotinas com "Casa" no nome
       'fit4less' — so rotinas com "Fit4Less" no nome
     Antes o casamento era por trecho do nome e pegava a primeira: uma
     rotina "... · Casa" criada no Hevy podia aparecer no lugar errado. */
  function acharRotina(chave, tipo){
    if(!chave || !HEVY || !HEVY.rotinas) return null;
    var alvo = semAcento(chave);
    var achou = Object.keys(HEVY.rotinas).filter(function(n){
      if(semAcento(n).indexOf(alvo) < 0) return false;
      var c = ehDeCasa(n), f = ehDaFit4less(n);
      if(tipo === 'casa') return c;
      if(tipo === 'fit4less') return f;
      return !c && !f;
    })[0];
  ...
  /* ══ ONDE VOCE TREINA HOJE: FIT4LESS OU CASA ══
     Decisoes do Luiz, 16/09/2026:
       · escolher na hora, com a Fit4Less como padrao; a escolha vale
         so para aquela data e fica neste aparelho
       · na Fit4Less, SO exercicio com aparelho que ele nao tem em casa.
         Ir a academia para fazer ponte de gluteo e bird dog nao faz
         sentido — foi o que a 06k mostrava, porque as 5 rotinas do
         Hevy sao todas de casa (peso do corpo, elastico, halter).

     De onde vem cada lista:
       Fit4Less → rotina do Hevy com "Fit4Less" no nome, se existir;
                  senao o programa FIT4LESS abaixo
       Casa     → rotina do Hevy com "Casa" no nome, se existir;
                  senao a rotina de sempre, passada pela adaptacao

     Equipamento de casa (fotos de 16/09): estacao com polia alta e
     ...
     Fit4Less (fit4less.ca): halteres de 5 a 75 lb, maquinas de forca,
     Smith, racks, remo, escada; cadeira de massagem nos horarios com
     equipe.                                                          */
  var LOCAL_PADRAO = 'fit4less';
  var KG_MAX_CASA = 22.7;                 // halter mais pesado: 50 lb
  var CHAVE_LOCAL = 'bq_local_';
  var FIM_PLIOMETRIA = '2026-09-20';      // regra da periodizacao acima

  /* A escolha fica guardada por data. Sem escolha: dia marcado como
     feito em casa abre em Casa; os outros, na Fit4Less.
     Antes a escolha de dia passado era apagada — mas o cartao do
     calendario agora mostra dias passados, e precisa lembrar onde
     voce treinou. Guardo 90 dias. */
  function localDe(iso){
    try{
      var v = localStorage.getItem(CHAVE_LOCAL + iso);
      if(v === 'casa' || v === 'fit4less') return v;
    }catch(e){}
    try{ if(typeof CASA_FEITA === 'object' && CASA_FEITA && CASA_FEITA[iso]) return 'casa' }catch(e){}
    return LOCAL_PADRAO;
  }

  function guardarLocal(iso, local){
    try{ localStorage.setItem(CHAVE_LOCAL + iso, local === 'casa' ? 'casa' : 'fit4less') }catch(e){}
  }

  try{
    var limite = new Date(); limite.setDate(limite.getDate() - 90);
    var lim = limite.getFullYear() + '-' + ('0' + (limite.getMonth() + 1)).slice(-2) + '-' + ('0' + limite.getDate()).slice(-2);
    var velhas = [];
  ...
  /* ══ PROGRAMA DA FIT4LESS ══
     Um por rotina da periodizacao, com o MESMO objetivo da rotina de
     casa correspondente, mas so com o que a academia tem e a casa nao:
     leg press, hack squat, Smith, cadeira de 45 graus, abdutora e
     adutora, panturrilha sentado e em pe, abdominal na maquina,
     cadeira romana, remo e escada.

     Criterios, para um maratonista de 64 anos a 4 semanas da prova:
       · 4 a 6 exercicios, 35 a 45 minutos
       · pesado so na Construcao; Reducao e Manutencao longe da falha
       · soleo sempre presente — e ele que segura o fim da maratona
       · agachamento pesado so em maquina guiada: seguro sem parceiro
       · pliometria (Jump Squat) so ate 20/09, e no aquecimento: e peso
  ...
  var FIT4LESS = {
    ...
    'quadril e core (base)': {
      aquecimento: '8 min na escada, leve',
      exercicios: [
        {nome:'Hip Thrust (Smith Machine)', series:'3×10', nota:'costas no banco, pausa de 1 s no alto; levante devagar'},
    ...
    'maxima (pico)': {
      aquecimento: '8 min no remo + 2 séries leves do Hack Squat',
      pliometria: 'Jump Squat 3×5 ao fim do aquecimento',
      exercicios: [
        {nome:'Hack Squat (Machine)', series:'4×5', nota:'pesado; 2 min de descanso entre as séries'},
        {nome:'Single Leg Press (Machine)', series:'3×6 cada', nota:'carga alta, desça em 2 s'},
        {nome:'Back Extension (Weighted Hyperextension)', series:'3×8', nota:'anilha no peito'},
        {nome:'Standing Calf Raise (Machine)', series:'4×8', nota:'carga alta'},
        {nome:'Crunch (Machine)', series:'3×10', nota:'ao sair da máquina, levante devagar'}
      ]},
    'quadril e core (pico)': {
      aquecimento: '6 min na escada, leve',
      exercicios: [
        {nome:'Hip Thrust (Smith Machine)', series:'3×8', nota:'carga moderada, longe da falha; levante devagar'},
  ...
  function programaFit4less(chave, iso, base){
    var p = FIT4LESS[semAcento(chave)];
    if(!p) return null;
    var aq = [p.aquecimento];
    if(p.pliometria && iso <= FIM_PLIOMETRIA) aq.push(p.pliometria);
    return {
      titulo: base && base.titulo ? base.titulo : chave,
      programa: 'fit4less',
      aquecimento: aq.join(' · '),
      exercicios: p.exercicios.map(function(e){
        var c = copiar(e); c.peso = 0; return c;
      })
    };
  }
  ...
  var TROCAS = [
    ...
    [/single leg press/, 'Bulgarian Split Squat (Dumbbell)', 'pé de trás no banco'],
    ...
    [/captain|crunch.*(machine|maquina)|abdominal.*(machine|maquina)/, 'Cable Crunch', 'polia alta da estação'],
  ...
  /* O que so existe na academia. Serve nos dois sentidos: a Casa troca
     tudo que casa aqui, e a Fit4Less so pode ter exercicio que casa
     aqui (conferido no teste e em bqLocal.conferir()).               */
  var SO_NA_ACADEMIA = /machine|maquina|smith|plate loaded|selectorized|leg press|hack squat|pendulum|belt squat|v squat|assisted|assistid|pec deck|butterfly|glute drive|rowing|ergometer|remo ergo|elliptical|eliptic|stair|escada|t bar row|chest supported|hyperextension|back extension|calf press|captain/;

  function precisaDeAcademia(nome){
    var t = semAcento(nome);
    return SO_NA_ACADEMIA.test(t) && !FICAM.test(t);
  }

  function trocaDeCasa(nome){
    var t = semAcento(nome);
    if(!precisaDeAcademia(nome)) return null;
  ...
  /* goblet e sempre com halter, mesmo quando o nome nao diz */
  function ehHalter(nome){ return /dumbbell|halter|goblet/.test(semAcento(nome)) }
  function ehPliometria(nome){ return /jump|pogo|\bhops?\b|salto|pliometr/.test(semAcento(nome)) }
  function ehAgachamentoComBarra(nome){
    var t = semAcento(nome);
    return /back squat|agachamento livre/.test(t) || (/squat/.test(t) && /barbell/.test(t));
  }
  ...
  /* Nao altera a rotina que veio do Hevy: devolve uma copia.
     Alem das trocas de aparelho, duas regras da periodizacao:
       · Maxima: agachamento pesado com barra nao se faz em casa, onde
         os suportes nao tem trava — vira bulgaro com halter
       · pliometria sai depois de 20/09                              */
  function adaptarParaCasa(r, chave, iso){
    var maxima = semAcento(chave).indexOf('maxima') >= 0;
    var c = copiar(r);
    c.adaptada = true;
    c.aquecimento = AQUECIMENTO_CASA;
    c.trocas = 0;
    c.exercicios = (r.exercicios || []).filter(function(e){
      return !(ehPliometria(e.nome) && iso > FIM_PLIOMETRIA);
    }).map(function(e){
      var novo = copiar(e);
      var tr = trocaDeCasa(e.nome);
      if(maxima && ehAgachamentoComBarra(e.nome)){
        tr = {nome: 'Bulgarian Split Squat (Dumbbell)',
              nota: 'agachamento pesado sem trava de segurança fica para a Fit4Less'};
      }
      if(tr && !tr.semPar){
        novo.nome = tr.nome;
        novo.antes = e.nome;
  ...
  function redesenhar(){
    pintar();
    ['renderCoach', 'renderCal', 'renderSemana', 'renderTreinos'].forEach(function(n){
  ...
  /* ══ O DIA DA ACADEMIA, NUM LUGAR SO ══
     ...
     da Fit4Less com o botao de CASA embaixo. Agora o mesmo bloco — onde,
  ...
  function rodapeDoTreino(r, local){
    ...
    if(local === 'casa' && r.adaptada && r.trocas)
      return 'Adaptada para os aparelhos de casa. Faça sem o Hevy.';
    if(local === 'casa' && r.adaptada) return 'Treino de casa: faça sem o Hevy.';
    if(local === 'casa') return 'Rotina de casa do seu Hevy.';
    if(r.programa === 'fit4less')
      return 'Só aparelhos que você não tem em casa. Esta rotina ainda não existe no Hevy: '
           + 'lá, comece um <b>treino vazio</b> e adicione estes exercícios.<br>' + massagem;
    return 'Rotina Fit4Less do seu Hevy.<br>' + massagem;
  }

  function dataCurta(iso){ return iso.slice(8, 10) + '/' + iso.slice(5, 7) }

  function botao(attrs, rotulo, tipo){
    var cheio = tipo === 'cheio', verde = tipo === 'verde';
    return '<button type="button" ' + attrs
      + ' style="flex:1;min-width:0;padding:10px 12px;border-radius:10px;font:inherit;'
      + 'font-size:13px;font-weight:800;cursor:pointer;'
      + 'border:1px solid ' + (verde ? '#3FD98A' : 'var(--gym,#9B6BD6)') + ';'
      + 'background:' + (cheio ? 'var(--gym,#9B6BD6)' : 'transparent') + ';'
      + 'color:' + (cheio ? '#fff' : verde ? '#3FD98A' : 'var(--gym,#9B6BD6)') + '">'
      + rotulo + '</button>';
  }

  /* situacao + botoes. Regras:
       feito no Hevy  → so a situacao (nada a fazer)
  ...
  function acoesDoDia(iso, local, exec){
    ...
    if(como === 'casa'){
      return '<div class="bqAcoes" style="margin-top:10px">'
        + '<div style="font-size:13px;font-weight:800;color:#3FD98A;margin-bottom:8px">✓ Feito em casa</div>'
        + '<div style="display:flex;gap:8px">' + botao('data-bqcasa="' + esc(iso) + '"', 'Desfazer', 'verde') + '</div></div>';
    }
    if(iso > hoje){
      s = 'Previsto para ' + dataCurta(iso) + '.';
      b = local === 'fit4less' ? botao('data-bqhevy="1"', 'Abrir o Hevy', 'vazio') : '';
    }else if(local === 'fit4less'){
      s = iso === hoje ? 'Pendente. Registre o treino no Hevy e depois toque em “Já registrei”.'
                       : 'Não registrado. Se treinou, registre no Hevy e toque em “Já registrei”.';
      b = botao('data-bqhevy="1"', 'Abrir o Hevy', 'vazio')
        + botao('data-bqpull="' + esc(iso) + '"', 'Já registrei no Hevy', 'cheio');
    }else{
      s = iso === hoje ? 'Pendente. Ao terminar, toque abaixo.' : 'Não registrado.';
      b = botao('data-bqcasa="' + esc(iso) + '"', 'Marcar como feito em casa', 'cheio');
    }
    return '<div class="bqAcoes" style="margin-top:10px">'
  ...
  function blocoDoDia(iso, r){
    var local = localDe(iso);
    if(!r){
      return seletorLocal(iso, local)
        + '<div class="bqa-u" style="font-size:12px;opacity:.7">A periodização não tem rotina para este dia.</div>'
        + acoesDoDia(iso, local);
    }
    var aquec = r.aquecimento
  ...
  function blocoSessao(){
    ...
    return '<div class="bqa-t">Hoje · 5:30 · ' + esc(r.titulo)
         + (como ? ' <span style="color:#3FD98A">✓ feito' + (como === 'casa' ? ' em casa' : '') + '</span>' : '') + '</div>'
         + blocoDoDia(iso, r);
  }

  /* para o cartao do calendario (parte "a aba coach e so da academia") */
  window.bqAcadCartao = function(iso){
    if(!/^\d{4}-\d{2}-\d{2}$/.test(String(iso))) return '';
    if(!HEVY) return '<div class="bqa-u" style="font-size:12px;opacity:.7">Carregando os dados do Hevy…</div>';
    var d = new Date(iso + 'T00:00:00');
    return blocoDoDia(iso, rotinaDoDia(diaSemana(d), iso));
  };

  /* ══ JA REGISTREI NO HEVY ══
     O hevy-pull roda sozinho a cada hora (minuto 20). Este botao nao
     espera: dispara o mesmo robo agora, acompanha, rele o Firebase e
     diz se achou o treino do dia. Mesmo caminho e mesmo token que o
     Sync Garmin usa. O robo so LE o Hevy. */
  var WF_PULL = 'hevy-pull.yml';
  var puxando = false;

  function tokenGH(){
    try{
      if(typeof GH_TOKEN === 'string' && GH_TOKEN) return GH_TOKEN;
      if(typeof cofre === 'object' && cofre && cofre.ler) return cofre.ler('gh_token') || '';
    }catch(e){}
    return '';
  }
  function repoGH(){
  ...
  function rotularTodos(iso, txt){
    try{
      document.querySelectorAll('[data-bqpull="' + iso + '"]').forEach(function(b){
  ...
  async function puxarHevy(iso){
    ...
    try{
      ...
      while(Date.now() - t0 < 5 * 60 * 1000){
        await espera(6000);
        try{
          var rr = await fetch(base + '/runs?event=workflow_dispatch&per_page=5&created=>=' + encodeURIComponent(desde), {headers: cab});
          if(!rr.ok) continue;
          var j = await rr.json();
          run = (j.workflow_runs || []).sort(function(a, c){ return new Date(c.created_at) - new Date(a.created_at) })[0] || null;
        }catch(e){ continue }
  ...
  function abrirHevy(){
    try{ window.open('https://hevy.com/', '_blank') }
    catch(e){ window.location.href = 'https://hevy.com/' }
  }

  /* Um ouvinte so, no documento: serve ao Coach e ao cartao do dia,
     que o app redesenha a toda hora. */
  ...
  window.bqLocal = {
    ...
    /* cada rotina de sempre: como fica na Fit4Less e em casa, hoje */
    relatorio: function(){
      if(!HEVY || !HEVY.rotinas) return 'Hevy ainda nao carregou';
      var hoje = hojeIso(), out = [];
      Object.keys(FIT4LESS).forEach(function(k){
        var base = acharRotina(k, 'base');
        var f = programaFit4less(k, hoje, base);
        var c = base ? adaptarParaCasa(base, k, hoje) : null;
        (f ? f.exercicios : []).forEach(function(e){
          out.push({rotina: k, local: 'Fit4Less', exercicio: e.nome, series: e.series});
        });
        (c ? c.exercicios : []).forEach(function(e){
          out.push({rotina: k, local: 'Casa', exercicio: e.nome, series: e.series,
                    obs: e.antes ? 'no lugar de ' + e.antes : (e.nota || '')});
        });
      });
  ...
  /* Para a aba GYM: um item por treino. Hevy traz data, titulo, min e
     kg (peso x repeticoes de todas as series). Casa conta so em dia
     sem Hevy, e nao tem carga. */
  window.bqGym = {
    pronto: function(){ return !!HEVY },
    sessoes: function(){
      var out = [], tem = {};
      sessoesHevy().forEach(function(s){
        if(!s || !s.data) return;
        tem[s.data] = 1;
        out.push({data: s.data, fonte: 'hevy', titulo: s.titulo || 'Treino no Hevy',
                  min: +s.min || 0, kg: +s.kg || 0});
      });
      Object.keys(CASA_FEITA || {}).forEach(function(iso){
        if(tem[iso] || !CASA_FEITA[iso]) return;
        out.push({data: iso, fonte: 'casa', titulo: 'Treino em casa',
                  min: +CASA_FEITA[iso].min || MIN_CASA, kg: 0});
      });
      return out;
    }
  };
...
/* ══════════════════════════════════════════════════════════════════════
   ABA GYM na aba Treinos (pedido do Luiz, 16/09/2026).
   O botao "Força" da fileira Tudo/Corrida/Bike/Natacao vira "GYM" e
   mostra a academia inteira, nao so o que o relogio gravou:

     Hevy     — cada treino registrado: minutos e peso total
                (carga x repeticoes de todas as series; o sync-hevy.py
                ja calcula e grava como "kg")
     Casa     — dias marcados com "feito em casa" (45 min, sem carga)
     Relogio  — sessao de forca do Garmin em dia sem Hevy nem casa

   Um treino por dia de fonte: dia com Hevy nao soma o relogio nem a
   casa de novo. Obedece ao Intervalo do alto da aba, como as outras.
   ══════════════════════════════════════════════════════════════════════ */
PARTE('aba gym', function(){
  if(typeof window.renderTreinos !== 'function') throw new Error('app sem renderTreinos');
  if(typeof ST !== 'object') throw new Error('sem ST');

  var ABERTOS = {};                       // meses abertos na lista
  var COR = 'var(--gym,#9B6BD6)';
  var ROTULO = {hevy: 'Hevy', casa: 'Casa', relogio: 'Relógio'};
  ...
  function diasAtras(k){ return Math.round((HOJE - dt(k)) / 86400000) }

  /* todos os treinos de academia, do mais novo para o mais antigo */
  function treinos(){
    var lista = (window.bqGym && window.bqGym.sessoes) ? window.bqGym.sessoes() : [];
    var porData = {};
    lista.forEach(function(s){ (porData[s.data] = porData[s.data] || []).push(s) });
    (ST.runs || []).forEach(function(r){
      if(!r || r.mod !== 'forca') return;
      var k = iso(addD(HOJE, -r.d));
      var min = (typeof duracaoDe === 'function' ? duracaoDe(r) : 0) / 60;
      var ja = porData[k];
      if(ja){
        /* o Hevy sem duracao aproveita a do relogio */
        ja.forEach(function(s){ if(!s.min && min) s.min = Math.round(min) });
        return;
      }
      var s = {data: k, fonte: 'relogio', titulo: r.titulo || 'Força (relógio)', min: Math.round(min) || 0, kg: 0};
      porData[k] = [s];
      lista.push(s);
    });
    return lista
      .filter(function(s){ var d = diasAtras(s.data); return d >= 0 && d < ST.periodo })
      .sort(function(a, b){ return a.data < b.data ? 1 : a.data > b.data ? -1 : 0 });
  }

  function trocarBotao(qtd){
    var b = document.querySelector('#tFiltros [data-f="forca"]') || document.querySelector('#tFiltros [data-f="gym"]');
    if(!b) return;
    b.setAttribute('data-f', 'gym');
    b.textContent = 'GYM' + (qtd ? ' ' + qtd : '');
    b.className = ST.filtro === 'gym' ? 'on' : '';
    b.disabled = false;
    b.style.opacity = qtd ? '' : '.35';
    b.onclick = function(){
      ST.filtro = 'gym';
      window.renderTreinos();
      try{ if(typeof persistir === 'function') persistir() }catch(e){}
    };
  }

  function painel(lista){
    var min = 0, kg = 0, n = {hevy: 0, casa: 0, relogio: 0};
    lista.forEach(function(s){ min += s.min || 0; kg += s.kg || 0; n[s.fonte]++ });
    var partes = [];
    if(n.hevy) partes.push(n.hevy + ' no Hevy');
    if(n.casa) partes.push(n.casa + ' em casa');
    if(n.relogio) partes.push(n.relogio + ' só no relógio');

    var html = '<div id="bqGymSub" style="font-size:11.5px;color:var(--tx3);margin:0 2px 12px;line-height:1.45">'
      + 'Últimos ' + ST.periodo + ' dias'
      + (partes.length ? ' · ' + partes.join(' · ') : '')
      + '. Peso total = carga × repetições de todas as séries registradas no Hevy.'
      + (ST.periodo > 182 ? ' O Hevy entrega aqui os últimos 6 meses.' : '')
      + '</div>';

    if(!lista.length){
      var carregando = window.bqGym && window.bqGym.pronto && !window.bqGym.pronto();
      return html + '<div class="vazio">' + (carregando ? 'Carregando os treinos do Hevy…' : 'Nenhum treino de academia neste intervalo.') + '</div>';
    }

    var meses = {};
    lista.forEach(function(s){ (meses[s.data.slice(0, 7)] = meses[s.data.slice(0, 7)] || []).push(s) });
    var chaves = Object.keys(meses).sort().reverse();
    var atual = iso(HOJE).slice(0, 7);
    if(!Object.keys(ABERTOS).length) ABERTOS[chaves.indexOf(atual) >= 0 ? atual : chaves[0]] = 1;

    html += chaves.map(function(k){
      var its = meses[k];
      var mm = its.reduce(function(a, s){ return a + (s.min || 0) }, 0);
      var kk = its.reduce(function(a, s){ return a + (s.kg || 0) }, 0);
      var ano = +k.slice(0, 4), mes = +k.slice(5, 7);
      var linhas = its.map(function(s){
        var d = dt(s.data);
        var sub = [];
        if(s.min) sub.push(s.min + ' min');
        sub.push(s.kg ? milhar(s.kg) + ' kg' : 'sem carga registrada');
        sub.push(ROTULO[s.fonte]);
        return '<div class="srow" style="cursor:default" data-gymdia="' + s.data + '">'
          + '<span class="sd"><b class="mono">' + d.getDate() + '</b><span>' + DIA3[dow(d) - 1] + '</span></span>'
          + '<span class="sl" style="background:' + COR + '"></span>'
          + '<span class="sb"><b>' + esc(s.titulo) + '</b><span>' + sub.join(' · ') + '</span></span>'
          + '<span class="sv"><b class="mono">' + (s.kg ? milhar(s.kg) : '—') + '</b><span>KG</span></span>'
          + '</div>';
      }).join('');
      return '<div class="wk2 ' + (ABERTOS[k] ? 'open' : '') + '">'
        + '<button class="wk2h" data-gymmes="' + k + '">'
        + '<span class="ch">›</span>'
        + '<span class="wq"><b>' + MES[mes - 1].replace(/^./, function(c){ return c.toUpperCase() }) + ' ' + ano + '</b>'
        + '<span>' + its.length + ' ' + (its.length === 1 ? 'treino' : 'treinos') + ' · ' + horas(mm) + '</span></span>'
        + '<span class="wt"><b class="mono">' + (kk ? milhar(kk) : '—') + '</b><span>KG</span></span>'
        + '</button>'
        + '<div class="wk2b">' + linhas + '</div></div>';
    }).join('');
    return html;
  }

  /* ── A CAIXA GYM ──
     Pedido do Luiz: os tres numeros sempre a vista na aba Treinos,
     logo abaixo do resumo de corrida — sem precisar tocar em nada.
     Tocar na caixa abre a lista do GYM. */
  function caixa(lista){
    var res = document.getElementById('tResumo');
    if(!res || !res.parentNode) return;
    var el = document.getElementById('bqGymCaixa');
    if(!el){
      el = document.createElement('div');
      el.id = 'bqGymCaixa';
      el.style.cssText = 'margin:0 0 14px;cursor:pointer';
      el.onclick = function(){
        ST.filtro = 'gym';
        window.renderTreinos();
        try{ if(typeof persistir === 'function') persistir() }catch(e){}
        var f = document.getElementById('tFiltros');
        if(f && f.scrollIntoView) f.scrollIntoView({behavior: 'smooth', block: 'start'});
      };
    }
    if(el.previousElementSibling !== res) res.parentNode.insertBefore(el, res.nextSibling);
    var min = 0, kg = 0;
    lista.forEach(function(s){ min += s.min || 0; kg += s.kg || 0 });
    el.innerHTML = '<div style="display:flex;justify-content:space-between;align-items:baseline;margin:0 2px 6px">'
      + '<span style="font-size:10.5px;font-weight:800;letter-spacing:.11em;text-transform:uppercase;color:' + COR + '">GYM</span>'
      + '<span style="font-size:10.5px;font-weight:600;color:var(--tx3)">últimos ' + ST.periodo + ' dias · ver lista ›</span></div>'
      + '<div class="resumo" id="bqGymRes" style="margin-bottom:0">'
      + '<div class="rz"><div class="k">Treinos</div><div class="v mono">' + lista.length + '</div></div>'
      + '<div class="rz"><div class="k">Horas</div><div class="v mono">' + (min / 60).toFixed(1) + '<small>h</small></div></div>'
      + '<div class="rz"><div class="k">Peso total</div><div class="v mono">' + milhar(kg) + '<small>kg</small></div></div>'
      + '</div>';
  }

  var antes = window.renderTreinos;
  window.renderTreinos = function(){
    if(ST.filtro === 'forca') ST.filtro = 'gym';      // o botao antigo virou este
    var r = antes.apply(this, arguments);
    try{
      var lista = treinos();
      caixa(lista);
      trocarBotao(lista.length);
      if(ST.filtro !== 'gym') return r;
      var alvo = document.getElementById('tSemanas');
      if(!alvo) return r;
      alvo.innerHTML = painel(lista);
      alvo.querySelectorAll('[data-gymmes]').forEach(function(b){
        b.onclick = function(){
          var k = b.getAttribute('data-gymmes');
          if(ABERTOS[k]) delete ABERTOS[k]; else ABERTOS[k] = 1;
          window.renderTreinos();
        };
      });
    }catch(e){ console.warn('aba gym:', e && e.message) }
    return r;
  };

  window.bqGymTela = { treinos: treinos };
  try{ if(document.getElementById('tFiltros')) window.renderTreinos() }catch(e){}
});
...
PARTE('ingles', function(){
  ...
  /* ── quinta leva: varredura do CÓDIGO-FONTE ──
     A primeira medição foi feita olhando a tela com dados de teste, e
     com poucos dados as caixas de Índices e Evolução mostram a mensagem
     de "não há corridas suficientes" em vez do texto explicativo. Por
     isso esses textos não entraram. Agora a lista saiu do código: cada
     literal do index.html e do fix.js, quebrado do mesmo jeito que o
     navegador quebra (em cada tag e em cada ${…}), apareça ele com
     dados ou sem.                                                     */
  var EN_FONTE = {
  'De':'From',
  'Só':'Only',
  ...
  'Agora não':'Not now',
  'Dia livre':'Free day',
  'Execução:':'How to do it:',
  'FC máxima':'Max HR',
  'Giro leve':'Easy spin',
  'Na fila (':'Queued (',
  'Não tenho':'I don\'t have one',
  'Onde dói?':'Where does it hurt?',
  'Segure em':'Hold at',
  'Sessão de':'Session of',
  'Seu ritmo':'Your pace',
  'Transição':'Transition',
  'bloco até':'block until',
  'de soneca':'of nap',
  'estável (':'steady (',
  'faixa boa':'good range',
  'janela de':'window of',
  'limítrofe':'borderline',
  'primeiro.':'first.',
  'sem dados':'no data',
  'semana de':'week of',
  'treino de':'workout of',
  'treinos ·':'workouts ·',
  '% a menos.':'% less.',
  'Agora está':'Now it is',
  'Até 30 min':'Up to 30 min',
  'Equilíbrio':'Balance',
  'FC máxima:':'Max HR:',
  'Leitura de':'Reading of',
  'Não feito:':'Not done:',
  'Rodagem de':'Easy run of',
  'Trote leve':'Easy jog',
  'Você dorme':'You sleep',
  'até agora.':'so far.',
  'carga alta':'heavy load',
  'exercícios':'exercises',
  'feito como':'done as',
  'g no total':'g in total',
  'ml de água':'ml of water',
  'no pace de':'at a pace of',
  'por noite.':'per night.',
  'quadríceps':'quads',
  '· vale até':'· valid until',
  'É opcional':'It is optional',
  '% do padrão':'% of standard',
  'Acelerações':'Strides',
  'Alimentação':'Fuelling',
  'Boa energia':'Good energy',
  'Cadência de':'Cadence of',
  'Composto em':'Built on',
  'Enquanto dá':'While you can',
  'Exercícios:':'Exercises:',
  'Manutenção.':'Maintenance.',
  'Nos últimos':'In the last',
  'Nova medida':'New reading',
  'Não achei o':'I could not find the',
  'O plano tem':'The plan has',
  'Ritmo médio':'Average pace',
  'Você correu':'You ran',
  'a partir de':'from',
  'dias ligada':'days on',
  'do esperado':'than expected',
  'em cerca de':'in about',
  'logo abaixo':'just below',
  'mg de sódio':'mg of sodium',
  'no lugar de':'instead of',
  'nos últimos':'in the last',
  'obrigatório':'required',
  'ritmo medio':'average pace',
  'trote entre':'jog between',
  '✓ Treino de':'✓ Workout of',
  '(VULNERÁVEL)':'(VULNERABLE)',
  '(automática)':'(automatic)',
  '6 km fáceis.':'6 easy km.',
  'Abaixo de Z1':'Below Z1',
  'Alvos do dia':'Targets for the day',
  'Bike no rolo':'Indoor trainer',
  'Controle bom':'Good control',
  'Cópia local.':'Local copy.',
  'Demonstração':'Demo',
  'Mover treino':'Move workout',
  'Não salvou (':'Did not save (',
  'Prova 2×4 km':'Race 2×4 km',
  'Prova 2×5 km':'Race 2×5 km',
  'Prova 3×3 km':'Race 3×3 km',
  'Prova 3×5 km':'Race 3×5 km',
  'Treinar para':'Train for',
  'Treino livre':'Free workout',
  'Você cumpriu':'You completed',
  'cancelada em':'cancelled on',
  'dia avaliado':'day assessed',
  'dia restante':'day left',
  'erro de rede':'network error',
  'esforço 8/10':'effort 8/10',
  'esforço 9/10':'effort 9/10',
  'faixa segura':'safe range',
  'glúteo médio':'glute medius',
  'km, ou seja,':'km, that is,',
  'seu normal ·':'your normal ·',
  'você cumpriu':'you completed',
  '· sem treino':'· no workout',
  'Último terço':'Final third',
  '/km (cerca de':'/km (about',
  '1× por semana':'once a week',
  '2× por semana':'twice a week',
  'Bike aeróbica':'Aerobic ride',
  'Café da manhã':'Breakfast',
  'Erro de rede:':'Network error:',
  'FC de repouso':'Resting HR',
  'Giro aeróbico':'Aerobic spin',
  'Inscrição até':'Registration until',
  'Jogo de ritmo':'Pace play',
  'Nova York, NY':'New York, NY',
  'Por que mudou':'Why it changed',
  'Stress diário':'Daily stress',
  'Teto do longo':'Long-run ceiling',
  'Treino manual':'Manual workout',
  'VO₂ do treino':'Workout VO₂',
  'Volta à calma':'Cool-down',
  'Zona aeróbica':'Aerobic zone',
  'de eletrólito':'of electrolyte',
  'dor ao correr':'pain when running',
  'glúteo máximo':'glute maximus',
  'km e você fez':'km and you did',
  'km por semana':'km per week',
  'lá, comece um':'there, start an',
  'mãos no banco':'hands on the bench',
  'nesta semana.':'this week.',
  'que no início':'than at the start',
  'segundo plano':'background',
  'sem treino em':'no workout on',
  '· no lugar de':'· instead of',
  '% das sessões,':'% of the sessions,',
  '% do previsto)':'% of plan)',
  '% mais devagar':'% slower',
  '. O ideal para':'. The ideal for',
  '3×8 cada perna':'3×8 each leg',
  'Afundo Búlgaro':'Bulgarian Split Squat',
  'Bloco contínuo':'Continuous block',
  'Bloco de ritmo':'Pace block',
  'Bloco em ritmo':'Block at pace',
  'Contrarrelógio':'Time trial',
  'Controle ótimo':'Excellent control',
  'Hora de compor':'Time to build',
  'Inscrição abre':'Registration opens',
  'Limiar na bike':'Threshold on the bike',
  'Nenhuma destas':'None of these',
  'Pace de limiar':'Threshold pace',
  'Primeiro terço':'First third',
  'Razão vertical':'Vertical ratio',
  'Ritmo de prova':'Race pace',
  'Ritmo maratona':'Marathon pace',
  'Stress elevado':'High stress',
  'Treino em casa':'Home workout',
  'bpm, abaixo de':'bpm, below',
  'dias avaliados':'days assessed',
  'hora de inicio':'start time',
  'km do previsto':'km of plan',
  'mede as fases.':'measures the stages.',
  'quando fechar.':'once it closes.',
  '— por causa de':'— because of',
  'Abaixo do ideal':'Below ideal',
  'Acima do limiar':'Above threshold',
  'Bike na estrada':'Road ride',
  'Bike — aeróbico':'Bike — aerobic',
  'Bloco 3 — forte':'Block 3 — hard',
  'Bloco de limiar':'Threshold block',
  'Carga da semana':'Week load',
  'Fechar o treino':'Close the workout',
  'Força (relógio)':'Strength (watch)',
  'Longão proposto':'Proposed long run',
  'Manter o treino':'Keep the workout',
  'OK para trocar.':'OK to swap.',
  'Olímpico · 5150':'Olympic · 5150',
  'Soltura na água':'Easy swim',
  'Série principal':'Main set',
  'Token do GitHub':'GitHub token',
  'Toques de ritmo':'Touches of pace',
  'Ver o resultado':'See the result',
  'com cadência em':'with cadence in',
  'e o fundo médio':'and the mid-range',
  'fica onde está.':'stays where it is.',
  'fora do Workout':'outside the Workout',
  'km para a faixa':'km to the range',
  'longos no ritmo':'long runs at pace',
  'nada marcado em':'nothing marked on',
  'no repositório.':'in the repository.',
  'prova já passou':'race already held',
  'sem treino hoje':'no workout today',
  '✓ Feito em casa':'✓ Done at home',
  ', FC subindo até':', HR rising to',
  '. Corrigido para':'. Corrected to',
  'Alongamento leve':'Light stretching',
  'Antes da largada':'Before the start',
  'Body Battery por':'Body Battery by',
  'Dia de recuperar':'Recovery day',
  'Guia de execução':'Execution guide',
  'Montar no Garmin':'Build on Garmin',
  'No mesmo pace de':'At the same pace of',
  'Outra modalidade':'Another sport',
  'Posição do corpo':'Body position',
  'Puxada na Frente':'Lat Pulldown',
  'Responder depois':'Answer later',
  'Ritmo de limiar:':'Threshold pace:',
  'Saúde e objetivo':'Health and goal',
  'Série de subidas':'Hill set',
  'Treino cancelado':'Workout cancelled',
  'Ver meus treinos':'See my workouts',
  'ajuste de rotina':'routine adjustment',
  'de carboidrato ·':'of carbohydrate ·',
  'média do período':'range average',
  's/km mais rápido':'s/km faster',
  '). Tente de novo.':'). Try again.',
  'A previsão piorou':'The projection got worse',
  'Arquivo inválido:':'Invalid file:',
  'Cadência por pace':'Cadence by pace',
  'Corrida logo após':'Run right after',
  'Fases do sono por':'Sleep stages by',
  'Força e prevenção':'Strength and prevention',
  'Mesmo movimento (':'Same movement (',
  'Média das últimas':'Average of the last',
  'Média do período:':'Range average:',
  'Nenhuma atividade':'No activity',
  'O ciclo começa em':'The cycle starts on',
  'Por que está aqui':'Why it is here',
  'Rodagem com prova':'Easy run with race pace',
  'Sim, quase sempre':'Yes, almost always',
  'Soltura pré-prova':'Pre-race shakeout',
  'Teste de nutrição':'Fuelling test',
  'Técnica na subida':'Uphill technique',
  'a partir dos seus':'from your',
  'que há três meses':'than three months ago',
  'ritmo de conversa':'conversational pace',
  'semanas até 18/10':'weeks to 18/10',
  'sessões no pacote':'sessions in the package',
  'tipo de atividade':'activity type',
  ', começando agora.':', starting now.',
  ', dentro do ruído)':', within the noise)',
  '3×40s + 3×30s cada':'3×40s + 3×30s each',
  'Cancelar o treino?':'Cancel the workout?',
  'Conectar ao Garmin':'Connect to Garmin',
  'Contato com o solo':'Ground contact',
  'Média semanal real':'Real weekly average',
  'Oscilação vertical':'Vertical oscillation',
  'Se algo der errado':'If something goes wrong',
  'Sim, já recuperado':'Yes, already recovered',
  'Treinos comparados':'Workouts compared',
  'a triagem de saúde':'the health screening',
  'agora tem plano de':'now has a plan of',
  'dentro do esperado':'within the expected range',
  'devolvido ao plano':'returned to the plan',
  'dia(s) liberado(s)':'day(s) freed',
  'dias o plano pediu':'days the plan asked for',
  'distância cumprida':'distance completed',
  'distâncias no site':'distances on the website',
  'dorsal e escápulas':'lats and scapulae',
  'tempo em movimento':'time moving',
  '% do volume, fechou':'% of volume, closed',
  ', começando daqui a':', starting in',
  '6 min no remo, leve':'6 min on the rower, easy',
  '8 min no remo, leve':'8 min on the rower, easy',
  'Alimentação na bike':'Fuelling on the bike',
  'Apagar esta medida?':'Delete this reading?',
  'Copiar para o MOTRA':'Copy to MOTRA',
  'Intervalado na bike':'Intervals on the bike',
  'Maior longo recente':'Longest recent long run',
  'Não consegui copiar':'I could not copy',
  'Por que este treino':'Why this workout',
  'Rodagem de entrada,':'Opening easy run,',
  'Sem registros desde':'No records since',
  'Sim, ainda incomoda':'Yes, still bothering me',
  'Supino com Halteres':'Dumbbell Bench Press',
  'Total porta a porta':'Total door to door',
  'core e anti-rotação':'core and anti-rotation',
  'depois da sincronia':'after the sync',
  'lesão no último ano':'injury in the last year',
  'panturrilha e sóleo':'calf and soleus',
  'planilha de treinos':'workout plan',
  'ponta do pé no step':'toe on the step',
  'posterior sob carga':'hamstrings under load',
  'pé de trás no banco':'back foot on the bench',
  'que as 8 anteriores':'than the previous 8',
  'seu volume atual de':'your current volume of',
  ', porque aqui não há':', because there is no',
  'Curto, mas aceitável':'Short, but acceptable',
  'Escolha seu objetivo':'Choose your goal',
  'Essa data já passou.':'That date has already passed.',
  'Hidratação e energia':'Hydration and energy',
  'Mediana da semana em':'Week median at',
  'Não consegui entrar:':'I could not sign in:',
  'Não consegui salvar:':'I could not save:',
  'Rodagem de polimento':'Taper easy run',
  'Salvar e sincronizar':'Save and sync',
  'batimento do coração':'heartbeat',
  'catálogo sem corrida':'catalogue without running',
  'glúteo, volume baixo':'glutes, low volume',
  'noites acima de 7 h.':'nights above 7 h.',
  'Última semana pesada':'Last heavy week',
  '% do volume planejado':'% of planned volume',
  '8 km + 6 acelerações.':'8 km + 6 strides.',
  '9 km + 6 acelerações.':'9 km + 6 strides.',
  'Aderência da quinzena':'Two-week adherence',
  'Bike na estrada 22 km':'Road ride 22 km',
  'Bike na estrada 25 km':'Road ride 25 km',
  'Bike na estrada 27 km':'Road ride 27 km',
  'Bike na estrada 29 km':'Road ride 29 km',
  'Bloco 2 — ritmo firme':'Block 2 — firm pace',
  'Como chamar essa data':'What to call that date',
  'Elas não voltam mais.':'They do not come back.',
  'Equilíbrio de contato':'Contact balance',
  'Km 32 ao fim — decida':'Km 32 to the end — decide',
  'Maior volume até aqui':'Biggest volume so far',
  'Nada a comparar ainda':'Nothing to compare yet',
  'Teto do próximo longo':'Next long-run ceiling',
  'Toque para atualizar.':'Tap to refresh.',
  'bpm, mas o relógio já':'bpm, but the watch already',
  'cada), FC subindo até':'each), HR rising to',
  'com cochilo, média de':'with nap, average of',
  'encolhe a cada bloco.':'shrinks with every block.',
  'força máxima de perna':'maximum leg strength',
  'km em ritmo de prova,':'km at race pace,',
  'lista antiga de volta':'old list back',
  'panturrilha com carga':'calf with load',
  'polia alta da estação':'high pulley on the cable station',
  '· hoje o plano propõe':'· today the plan proposes',
  '— a que você informou':'— the one you entered',
  '— estimada pela idade':'— estimated from your age',
  ', seu coração trabalha':', your heart works',
  'Brick — bike + corrida':'Brick — bike + run',
  'Como está no seu Hevy.':'As it is in your Hevy.',
  'Esteira com inclinação':'Treadmill with incline',
  'Mover o segundo treino':'Move the second workout',
  'Nenhuma atividade sua.':'No activity of yours.',
  'Por que sem cronômetro':'Why with no stopwatch',
  'Pratica outro esporte?':'Do you do another sport?',
  'Recalibração quinzenal':'Two-week recalibration',
  'abaixo do alvo de 3:50':'below the 3:50 target',
  'ao chamar o Hevy Pull.':'when calling Hevy Pull.',
  'após carregar os dados':'after loading the data',
  'baixo e para a direita':'down and to the right',
  'elasticidade do tendão':'tendon elasticity',
  'km em rodagem, últimos':'km of easy running, last',
  'peito, ombro e tríceps':'chest, shoulder and triceps',
  'polia baixa da estação':'low pulley on the cable station',
  'quadríceps, unilateral':'quads, single leg',
  '— (ainda não composto)':'— (not built yet)',
  '. Nada a cumprir ainda.':'. Nothing to complete yet.',
  ': faça antes de apertar':': do this before pressing',
  'Falta o treino de força':'The strength workout is missing',
  'Hevy ainda nao carregou':'Hevy has not loaded yet',
  'M\\u00e9dia por m\\u00eas':'Average per month',
  'Maratona aquática 10 km':'Open-water 10 km',
  'Nova versão disponível.':'New version available.',
  'Não consegui atualizar:':'I could not update:',
  'Não há plano registrado':'No plan on record',
  'Por que descansar tanto':'Why rest this much',
  'Saúde e condicionamento':'Health and fitness',
  'Solto do começo ao fim.':'Relaxed from start to finish.',
  'Soltura com acelerações':'Shakeout with strides',
  'Sua história na corrida':'Your running history',
  'Você correu na quinzena':'You ran in the fortnight',
  'correção de altimetria.':'elevation correction.',
  'halter sobre os joelhos':'dumbbell over the knees',
  '— relendo as atividades':'— re-reading the activities',
  '. Último longo do ciclo.':'. Last long run of the cycle.',
  '5 a 6 tiros de 1000 m em':'5 to 6 reps of 1000 m at',
  'Acelerações progressivas':'Progressive strides',
  'Longo de pernas cansadas':'Long run on tired legs',
  'Longo em pernas cansadas':'Long run on tired legs',
  'Nenhum objetivo definido':'No goal set',
  'O que levar neste treino':'What to take on this workout',
  'O que mudou no seu plano':'What changed in your plan',
  'Pace certo, mas faltaram':'Right pace, but short of',
  'está no mesmo peso desde':'has been at the same weight since',
  'glúteo máximo e potência':'glute max and power',
  'km em ritmo de maratona,':'km at marathon pace,',
  'km em rodagem tranquila.':'km of easy running.',
  'lesão que ainda incomoda':'injury that still bothers me',
  'sem semanas para mostrar':'no weeks to show',
  'sessão sem quilometragem':'session with no distance',
  '20 km soltos e os últimos':'20 km relaxed and the last',
  '22 km soltos e os últimos':'22 km relaxed and the last',
  'Buscando as medidas\\u2026':'Fetching the readings…',
  'O GitHub não achou o robô':'GitHub could not find the robot',
  'Preencha os dois números.':'Fill in both numbers.',
  'Sem dados para recalibrar':'No data to recalibrate',
  'Volume alto, cabeça calma':'High volume, calm head',
  'a maratona sai hoje entre':'the marathon comes out today between',
  'com aparelhos da Fit4Less':'with Fit4Less machines',
  'liga o plano da maratona.':'turns the marathon plan on.',
  'nenhuma sessão com marcas':'no session with marks',
  'no chão, sem banco romano':'on the floor, no roman chair',
  'quadríceps, glúteo e core':'quads, glutes and core',
  'Bike leve na estrada 14 km':'Easy road ride 14 km',
  'Bike leve na estrada 18 km':'Easy road ride 18 km',
  'Cai o volume, o ritmo fica':'Volume drops, pace stays',
  'Conversa do início ao fim.':'Conversation from start to finish.',
  'Eficiência aeróbica caindo':'Aerobic efficiency falling',
  'O ciclo ainda não começou.':'The cycle has not started yet.',
  'Sem atividades no servidor':'No activities on the server',
  'Sistólica · o número maior':'Systolic · the higher number',
  'com o que você tem em casa':'with what you have at home',
  'elástico acima dos joelhos':'band above the knees',
  'não consegui criar o bloco':'could not create the block',
  'posterior de coxa e glúteo':'hamstrings and glutes',
  'Arquivo de backup inválido.':'Invalid backup file.',
  'Conecte o GitHub pelo botão':'Connect GitHub with the button',
  'Diastólica · o número menor':'Diastolic · the lower number',
  'Eficiência aeróbica subindo':'Aerobic efficiency rising',
  'Escuro, claro, ou Auto para':'Dark, light, or Auto to',
  'Há quanto tempo você corre?':'How long have you been running?',
  'Longo 16 km · 6 km de prova':'Long run 16 km · 6 km at race pace',
  'Longo 28 km · 8 km de prova':'Long run 28 km · 8 km at race pace',
  'Não consegui ler o arquivo.':'I could not read the file.',
  'O sync terminou com falha (':'The sync finished with a failure (',
  'Rodagem fácil rápida demais':'Easy run too fast',
  'Rodagem leve + tiros curtos':'Easy run + short reps',
  'Tempo disponível por sessão':'Time available per session',
  'Token recusado. Gere outro.':'Token refused. Generate another one.',
  'registros do outro aparelho':'records from the other device',
  'sessões de força recriadas.':'strength sessions recreated.',
  'sessões de força removidas.':'strength sessions removed.',
  'Conversa fluida o tempo todo':'Easy conversation the whole way',
  'Demorou mais que o esperado.':'It took longer than expected.',
  'Fases do sono · última noite':'Sleep stages · last night',
  'Longo 26 km · 12 km de prova':'Long run 26 km · 12 km at race pace',
  'Longo 32 km · 10 km de prova':'Long run 32 km · 10 km at race pace',
  'No mesmo batimento você está':'At the same heart rate you are',
  'Pausa de 2 segundos embaixo.':'2-second pause at the bottom.',
  'Por DISTÂNCIA. Alvo de ritmo':'By DISTANCE. Pace target',
  'arquivo sem a lista "provas"':'file without the "provas" list',
  'então não há o que comparar.':'so there is nothing to compare.',
  'nenhuma corrida nos próximos':'no run in the next',
  'sim — nunca conta como feito':'yes — never counts as done',
  '. Mostrando dados de exemplo.':'. Showing sample data.',
  '. Não dá tempo de um ciclo de':'. There is not enough time for a cycle of',
  '5×100 m em ritmo de conversa.':'5×100 m at conversational pace.',
  'Dias disponíveis para treinar':'Days available to train',
  'E-mail ou senha não conferem.':'E-mail or password do not match.',
  'Hoje não tem. Próxima sessão:':'Nothing today. Next session:',
  'No formato hh:mm:ss ou mm:ss.':'In hh:mm:ss or mm:ss format.',
  'Sessões de força automáticas:':'Automatic strength sessions:',
  'Sono curto limita a adaptação':'Short sleep limits adaptation',
  'carga do pico, longe da falha':'peak load, far from failure',
  'na maratona, contra o alvo de':'in the marathon, against the target of',
  'perna unilateral e equilíbrio':'single leg and balance',
  'plano inteiro visível de novo':'whole plan visible again',
  'você cumpriu o volume inteiro':'you completed the whole volume',
  '— calculado a partir dos seus':'— calculated from your',
  '. Até lá, mantenha base livre.':'. Until then, keep the base easy.',
  '/km. Confortavelmente difícil.':'/km. Comfortably hard.',
  'Já participou de alguma prova?':'Have you ever done a race?',
  'base de pernas para a maratona':'leg base for the marathon',
  'bpm. Confortavelmente difícil:':'bpm. Comfortably hard:',
  'força de empurrar com controle':'pushing strength with control',
  'mesmo tempo, mesma intensidade':'same time, same intensity',
  'nenhum dia com força cancelada':'no day with strength cancelled',
  'nenhuma incoerência encontrada':'no inconsistency found',
  'pausa de 1 s embaixo, alongado':'1 s pause at the bottom, stretched',
  'quilômetro mais rápido medido:':'fastest kilometre measured:',
  's/km mais rápido que o pedido.':'s/km faster than asked.',
  'sem nutrição marcada neste dia':'no fuelling marked on this day',
  '— estimado pela sua rodagem de':'— estimated from your easy run of',
  'Manutenção. Amplitude completa.':'Maintenance. Full range of motion.',
  'Poucos dias para julgar o bloco':'Too few days to judge the block',
  'Problema cardíaco diagnosticado':'Diagnosed heart problem',
  'Sessões fortes não estão saindo':'Hard sessions are not coming out',
  'Somar 2 km ao próximo longo, de':'Add 2 km to the next long run, from',
  'dinâmicos e os educativos ficam':'dynamic ones and the drills stay',
  'e sua eficiência aeróbica subiu':'and your aerobic efficiency rose',
  'glúteo, quadríceps e equilíbrio':'glutes, quads and balance',
  'o sync do Hevy ainda não gravou':'the Hevy sync has not saved yet',
  's/km mais devagar que o pedido.':'s/km slower than asked.',
  '/km, não para o padrão de elite.':'/km, not to the elite standard.',
  '400 m soltos + 200 m de pernada.':'400 m relaxed + 200 m kick.',
  'Bloco contínuo de 20 a 25 min em':'Continuous block of 20 to 25 min at',
  'Escolha um objetivo na aba Coach':'Choose a goal on the Coach tab',
  'Nenhuma prova com esses filtros.':'No race matches these filters.',
  'No ritmo e na distância pedidos.':'At the pace and distance asked for.',
  'O plano não tem nenhum dia entre':'The plan has no day between',
  'Seu ritmo de rodagem confortável':'Your comfortable easy-run pace',
  'no leg press, amplitude completa':'on the leg press, full range of motion',
  'sessão de força não identificada':'unidentified strength session',
  '8 min na bike ou na esteira, leve':'8 min on the bike or treadmill, easy',
  'A sincronia roda de hora em hora.':'The sync runs every hour.',
  'Base aeróbica, onde mora o volume':'Aerobic base, where the volume lives',
  'Começa o bloco que decide a prova':'The block that decides the race begins',
  'Falha na autenticação do Firebase':'Firebase authentication failed',
  'N\\u00e3o consegui ler as medidas:':'I could not read the readings:',
  'No caminho, com pontos a corrigir':'On track, with points to fix',
  'Não consegui alcançar o servidor:':'I could not reach the server:',
  'Não consegui falar com o GitHub (':'I could not reach GitHub (',
  'Sem impacto, soma volume aeróbico':'No impact, adds aerobic volume',
  'Sessão sem detalhamento definido.':'Session with no detail defined.',
  'glúteo médio, estabiliza o joelho':'glute medius, stabilises the knee',
  'na máquina de panturrilha sentado':'on the seated calf machine',
  'tempo de contato com o solo medio':'average ground contact time',
  'Costelas para baixo, glúteo ativo.':'Ribs down, glutes engaged.',
  'Faixa calculada para o seu pace de':'Range calculated for your pace of',
  'Faz musculação ou treino de força?':'Do you do weights or strength training?',
  'Não consegui ler os dados do Hevy:':'I could not read the Hevy data:',
  'Salva ou restaura tudo num arquivo':'Saves or restores everything in one file',
  'Suba completo e desça bem devagar.':'Rise all the way up and lower very slowly.',
  'carga do pico, 2×5, longe da falha':'peak load, 2×5, far from failure',
  'nenhum treino chegou do Hevy ainda':'no workout has arrived from Hevy yet',
  '. Ele fica salvo só neste aparelho.':'. It is saved on this device only.',
  '/km, com 3 min de trote entre eles.':'/km, with 3 min jog between them.',
  'Lombar colada no chão o tempo todo.':'Lower back flat on the floor throughout.',
  'Por TEMPO, não por distância. Ritmo':'By TIME, not by distance. Pace',
  'Sente dor ou desconforto ao correr?':'Do you feel pain or discomfort when running?',
  'Troque pela data real da sua prova.':'Replace it with your real race date.',
  'desligado — plano original de volta':'off — original plan back',
  'volta controlada, sem soltar o peso':'controlled return, without dropping the weight',
  '5 km soltos + 4 acelerações de 20 s.':'5 km relaxed + 4 strides of 20 s.',
  '8 km soltos + 6 acelerações de 20 s.':'8 km relaxed + 6 strides of 20 s.',
  'A semana mais dura — e a última dura':'The hardest week — and the last hard one',
  'Nenhuma atividade válida encontrada.':'No valid activity found.',
  'O robô Hevy Pull terminou com erro (':'The Hevy Pull robot finished with an error (',
  'Pace por corrida com mediana semanal':'Pace per run with weekly median',
  'Puxe com os cotovelos, peito aberto.':'Pull with your elbows, chest open.',
  'acima de 7 h somando noite e soneca.':'above 7 h counting night and nap.',
  'km reais das duas semanas anteriores':'actual km over the two previous weeks',
  '📋 Refazer o questionário do corredor':'📋 Retake the runner questionnaire',
  '% mais rápido do que há três semanas.':'% faster than three weeks ago.',
  '(Finalizar › Salvar) e toque de novo.':'(Finish › Save) and tap again.',
  'Desmarcar o treino de casa deste dia?':'Unmark the home workout for this day?',
  'Desça até o peito alto, sem balançar.':'Lower to the upper chest, without swinging.',
  'No mesmo batimento você está correndo':'At the same heart rate you are running',
  'Não identifiquei a sessão desta tela.':'I could not identify the session on this screen.',
  'Preencha tudo. Paces no formato m:ss.':'Fill in everything. Paces in m:ss format.',
  'Primeiro contato com o ritmo de prova':'First contact with race pace',
  'Resistência, o treino mais importante':'Endurance, the most important session',
  '). No GitHub: aba Actions › Hevy Pull.':'). On GitHub: Actions tab › Hevy Pull.',
  'Erro 403. O token precisa da permissão':'Error 403. The token needs the permission',
  'Erro 403. O token precisa de permissão':'Error 403. The token needs permission',
  'O plano está passando por cima de você':'The plan is running you over',
  'Pressão alta sem acompanhamento médico':'High blood pressure without medical follow-up',
  'Sistólica fora do que o aparelho mede.':'Systolic outside what the device measures.',
  'equilibrio de contato com o solo medio':'average ground contact balance',
  'min de trote leve. Nao pare de correr.':'min of easy jog. Do not stop running.',
  '. Segure aqui: o trabalho vem a seguir.':'. Hold here: the work comes next.',
  'Core segura a postura no quilômetro 35.':'Core holds your posture at kilometre 35.',
  'Diastólica fora do que o aparelho mede.':'Diastolic outside what the device measures.',
  'O que o treinador agenda no Garmin traz':'What your coach schedules in Garmin carries',
  'Pelo seu melhor esforço longo recente (':'From your best recent long effort (',
  'Step Speed Loss · freada a cada passada':'Step Speed Loss · braking on every stride',
  'Suba o quadril com pausa de 1s no topo.':'Lift the hips with a 1 s pause at the top.',
  'Sync concluído. Atividades atualizadas.':'Sync finished. Activities updated.',
  'Zero impacto. Ajuda a soltar as pernas.':'Zero impact. Helps loosen the legs.',
  'Escolha a distância que você vai correr.':'Choose the distance you are going to run.',
  'Escolha a prova para o plano ser montado':'Choose the race so the plan can be built',
  'Forte, respiração pesada, trechos curtos':'Hard, heavy breathing, short stretches',
  'Não havia treino previsto para este dia,':'There was no workout planned for this day,',
  'O objetivo está longe do que você já fez':'The goal is far from what you have done',
  'Seu melhor esforço recente projeta entre':'Your best recent effort projects between',
  'anos o tendão avisa depois, não durante.':'the tendon warns you afterwards, not during.',
  'você marcou dor ou lesão no questionário':'you reported pain or injury in the questionnaire',
  'Alguma destas situações se aplica a você?':'Does any of these apply to you?',
  'O arquivo está vazio ou só tem cabeçalho.':'The file is empty or has only a header.',
  'Quantas vezes por semana você corre hoje?':'How many times a week do you run now?',
  'Sem data escolhida, o quadro não aparece.':'With no date chosen, the panel does not appear.',
  '— seu histórico comporta o ciclo completo':'— your history supports the full cycle',
  '. Rode o Hevy Pull no GitHub e recarregue.':'. Run Hevy Pull on GitHub and reload.',
  'Cada barra vai do fundo ao pico de energia':'Each bar runs from the low to the peak of energy',
  'Descarga — resista à vontade de fazer mais':'Unload — resist the urge to do more',
  'Mesma técnica, carga alta, longe da falha.':'Same technique, heavy load, far from failure.',
  'O robô leu o Hevy, mas lá não há treino em':'The robot read Hevy, but there is no workout there on',
  'Por TEMPO, trote leve. Não pare de correr.':'By TIME, easy jog. Do not stop running.',
  'Zero impacto, trabalha respiração e ombros':'Zero impact, works breathing and shoulders',
  'educativos ANTES do start, relogio parado.':'drills BEFORE the start, watch stopped.',
  'km um pouco mais forte, ainda confortável.':'km a little harder, still comfortable.',
  'nesse dia, fora da noite. Somando as duas,':'on that day, outside the night. Adding the two,',
  '— os trechos fortes podem ter ficado leves':'— the hard stretches may have come out easy',
  'As duas marcas em cada barra são a faixa de':'The two marks on each bar are the range of',
  'Empurre o quadril para trás, coluna neutra.':'Push the hips back, spine neutral.',
  'Frequência cardíaca por pace, dois períodos':'Heart rate by pace, two ranges',
  'Sincronizado · 1 registro do outro aparelho':'Synced · 1 record from the other device',
  'Tem tontura, desmaio ou perda de equilíbrio':'Do you have dizziness, fainting or loss of balance',
  'km bem soltos, carga baixa e cadência alta.':'km very relaxed, low load and high cadence.',
  '. Voce deve conseguir falar frases inteiras.':'. You should be able to speak full sentences.',
  'A periodização não tem rotina para este dia.':'The periodisation has no routine for this day.',
  'O tempo de maratona que o seu melhor esforço':'The marathon time that your best effort',
  'Token recusado. Gere outro pelo Sync Garmin.':'Token refused. Generate another one via Sync Garmin.',
  'km por semana no último mês. A razão está em':'km per week in the last month. The ratio is at',
  'seu halter vai até 50 lb: suba as repetições':'your dumbbell goes up to 50 lb: raise the reps',
  'treinos de qualidade bateram distância e FC.':'quality workouts matched distance and heart rate.',
  'Academia às segundas, quartas e sextas, 5:30.':'Gym on Mondays, Wednesdays and Fridays, 5:30.',
  'Confortavelmente difícil, 40 min sustentáveis':'Comfortably hard, 40 min sustainable',
  'Glúteo fraco vira dor lombar no fim do longo.':'Weak glutes become low-back pain late in the long run.',
  'Maior distância que você já correu de uma vez':'The longest distance you have ever run at once',
  'Quantas horas você dorme por noite, em média?':'How many hours do you sleep per night, on average?',
  'banco com suportes, sem trava: carga moderada':'bench with supports, no safety catch: moderate load',
  'desliga o plano e volta ao automático do app.':'turns the plan off and returns the app to automatic.',
  '/km. É o ritmo que você quer sentir no dia 18.':'/km. This is the pace you want to feel on the 18th.',
  'A · 3:50:00 (5:27/km) — folga larga no índice.':'A · 3:50:00 (5:27/km) — wide margin on the qualifier.',
  'Dias em destaque ainda não têm segundo treino.':'Highlighted days do not have a second workout yet.',
  'Distribui\\u00e7\\u00e3o \\u00b7 todas as medidas':'Distribution · all readings',
  'No degrau, descendo bem abaixo da linha do pé.':'On the step, dropping well below the line of the foot.',
  'Quantos quilômetros por semana, mais ou menos?':'How many kilometres per week, roughly?',
  'Rodagem leve com 4 acelerações de 20 s no fim.':'Easy run with 4 strides of 20 s at the end.',
  'a primeira semana com plano registrado fechar.':'the first week with a plan on record closes.',
  'joelho alinhado com o pé, sem cair para dentro':'knee in line with the foot, not collapsing inwards',
  'km bem leves. Nada de testar ritmo na véspera.':'very easy km. No pace testing the day before.',
  'km um pouco mais firmes, sem chegar a difícil.':'km a little firmer, without becoming hard.',
  '. Se chegar ofegante ao fim, foi rapido demais.':'. If you arrive out of breath, it was too fast.',
  'Chegar descansado vale mais que qualquer treino':'Arriving rested is worth more than any workout',
  'Desça com controle, suba sem travar o cotovelo.':'Lower under control, press up without locking the elbow.',
  'Tirar 15% das rodagens fáceis da próxima semana':'Take 15% off next week\'s easy runs',
  'Toma medicação contínua para coração ou pressão':'Do you take ongoing medication for heart or blood pressure',
  'A previsão está estável desde o começo do ciclo.':'The projection has been steady since the cycle began.',
  'Converse com um médico antes de aumentar a carga':'Talk to a doctor before increasing the load',
  'Esse mês está fora do intervalo escolhido acima.':'That month is outside the range chosen above.',
  'Um médico já disse para você não fazer exercício':'Has a doctor ever told you not to exercise',
  'aquiles e sóleo, onde o veterano mais se machuca':'achilles and soleus, where the veteran gets hurt most',
  'nos suportes, sem trava: carga que você controla':'on the supports, no catch: a load you control',
  'semanas entram aqui. Antes eu olhava o histórico':'weeks count here. Before, I looked at the whole history',
  'terreno: descida infla este número por natureza.':'terrain: downhill inflates this number by nature.',
  '· copie e cole no MOTRA para programar a sessão.':'· copy and paste into MOTRA to program the session.',
  '— passos mais curtos e frequentes, sem acelerar.':'— shorter, more frequent steps, without speeding up.',
  'Caminhe alto e estável, sem tombar para os lados.':'Walk tall and steady, without tipping side to side.',
  'Essa data ainda não chegou — o quadro fica vazio.':'That date has not arrived yet — the panel stays empty.',
  'Informe o tempo alvo no formato h:mm:ss ou mm:ss.':'Enter the target time in h:mm:ss or mm:ss format.',
  'Subida controlada, sem impulso da perna de baixo.':'Controlled rise, without pushing off the bottom leg.',
  'km que os blocos pediram nas semanas já fechadas.':'km the blocks called for in the weeks already closed.',
  'rodagens fáceis saíram bem acima do ritmo pedido.':'easy runs came out well above the pace asked for.',
  '— o mesmo número do cartão acima. Os alongamentos':'— the same number as the card above. The stretches',
  'Confortavelmente difícil, 20 a 25 min sustentáveis':'Comfortably hard, 20 to 25 min sustainable',
  'Costelas para baixo, glúteo ativo, pescoço neutro.':'Ribs down, glutes engaged, neck neutral.',
  'Largada nos Estados Unidos, chegada nas cataratas.':'Start in the United States, finish at the falls.',
  'Semana leve também na água. 5×100 m soltos, e fim.':'Easy week in the water too. 5×100 m relaxed, and done.',
  'corridas, ponderada por distância · cadência média':'runs, weighted by distance · average cadence',
  'Controle a descida, joelho não colapsa para dentro.':'Control the descent, the knee does not collapse inwards.',
  'Descanso hoje — a fase atual não prevê sessão nesta':'Rest today — the current phase has no session on this',
  'É exatamente o que a fase de base deveria produzir.':'It is exactly what the base phase should produce.',
  'Desça controlando, tronco firme e joelhos alinhados.':'Lower under control, torso firm and knees aligned.',
  'Já sentiu dor no peito em repouso ou fazendo esforço':'Have you had chest pain at rest or during effort',
  'O GitHub recusou (403): o token precisa da permissão':'GitHub refused (403): the token needs the permission',
  'Prova ou treino forte, tanto faz. Pule se não tiver.':'A race or a hard workout, either one. Skip if you have none.',
  'e todas as faixas de FC saem baixas por causa disso.':'and every HR range comes out low because of that.',
  'min de trote muito leve ate a respiracao normalizar.':'min of very easy jog until your breathing settles.',
  'Crie um bloco de repetição com os dois passos abaixo.':'Create a repeat block with the two steps below.',
  'Curto de propósito. Corpo cansado não absorve treino.':'Short on purpose. A tired body does not absorb training.',
  'É o que evita o joelho cair para dentro com a fadiga.':'It is what keeps the knee from collapsing inwards with fatigue.',
  'A corrida fica no relógio, montada pelo seu treinador.':'Running lives on the watch, built by your coach.',
  'Este ritmo só existe dentro dos tiros — nunca corra os':'This pace exists only inside the reps — never run the',
  'Nenhuma corrida com o dado ainda. Ele precisa da cinta':'No run with that data yet. It needs the strap',
  ', e mede o quanto você acelerou em relação ao que o seu':', and it measures how much you sped up compared with what your',
  'Ainda não há semana com plano registrado para comparar.':'There is no week with a plan on record to compare yet.',
  'Este dia ainda não tem treino porque o plano só vai até':'This day has no workout yet because the plan only runs to',
  'pelo Atalho \\u2014 o mesmo caminho dos treinos manuais.':'through the Shortcut — the same path as manual workouts.',
  'seguir o ajuste do iPhone e virar sozinho ao anoitecer.':'follow the iPhone setting and switch by itself at dusk.',
  'É o ritmo que você quer sentir no dia 18. O restante em':'It is the pace you want to feel on the 18th. The rest at',
  '4 × 25 m no ritmo de prova, com 30 segundos de descanso.':'4 × 25 m at race pace, with 30 seconds rest.',
  '70 min em Z2, com 300 m de subida acumulada se possível.':'70 min in Z2, with 300 m of climbing if possible.',
  'Backup restaurado sem perder nada do que já estava aqui.':'Backup restored without losing anything that was already here.',
  'HRM 600 e chega junto com a próxima sincronia do Garmin.':'HRM 600 and arrives with the next Garmin sync.',
  'Os quilômetros da última semana divididos pela sua média':'The kilometres of the last week divided by your average',
  'Por TEMPO. Trote muito leve até a respiração normalizar.':'By TIME. Very easy jog until your breathing settles.',
  'Potência aeróbica sobre a bike. Eleva o teto de esforço.':'Aerobic power on the bike. Raises your effort ceiling.',
  'minutos de trote muito leve até a respiração normalizar.':'minutes of very easy jog until your breathing settles.',
  'minutos em trote muito leve até a respiração normalizar.':'minutes of very easy jog until your breathing settles.',
  'máquina que não existe em casa: faça com halter ou polia':'a machine you don\'t have at home: use a dumbbell or cable',
  '/km — se isso parecer rápido demais, aumente este número.':'/km — if that feels too fast, raise this number.',
  'você fala 3 ou 4 palavras, não uma frase. Antes e depois,':'you can say 3 or 4 words, not a sentence. Before and after,',
  '. Confira em Configurações — as faixas de FC dependem dela.':'. Check it in Settings — the HR ranges depend on it.',
  '4 km bem leves. Retire o número e deixe tudo separado hoje.':'4 very easy km. Pick up your bib and lay everything out today.',
  ': a prova larga às 7h e todos os seus treinos são de tarde.':': the race starts at 7 a.m. and all your workouts are in the afternoon.',
  'Combina bem com um dia leve, e preserva seu dia de descanso':'It pairs well with an easy day, and it protects your rest day',
  'Descida em 3 segundos, subida firme. Joelho na linha do pé.':'Three seconds down, firm on the way up. Knee in line with the foot.',
  'Informe a data da próxima prova para o plano ser remontado.':'Enter the date of the next race so the plan can be rebuilt.',
  'OK desliga e remove todas as sessões de força que eu criei.':'OK turns it off and removes every strength session I created.',
  'de academia ainda sem conferência: os treinos do Hevy estão':'gym sessions not yet checked: the Hevy workouts are',
  'seu melhor esforço da quinzena projeta um limiar mais lento':'your best effort of the fortnight projects a slower threshold',
  'Ainda não há esforço recente forte o bastante para projetar.':'There is no recent effort hard enough to project from yet.',
  'O GitHub recusou o token (401). Gere outro e cole pelo botão':'GitHub refused the token (401). Generate another and paste it with the button',
  'duração e distância, mas não ritmo — por isso os travessões.':'duration and distance, but not pace — hence the dashes.',
  'mantenha, e use este painel só para perceber se algo piorar.':'keep it up, and use this panel only to notice if something gets worse.',
  'min de trote leve entre eles. Aquecimento e volta à calma em':'min of easy jog between them. Warm-up and cool-down at',
  'seu melhor esforço da quinzena projeta um limiar mais rápido':'your best effort of the fortnight projects a faster threshold',
  'Um único Workout, do começo ao fim. O relógio grava tudo como':'A single Workout, start to finish. The watch records it all as',
  'estímulo de fibra rápida, a lacuna que a auditoria encontrou.':'fast-fibre stimulus, the gap the audit found.',
  'o chão. A pliometria das semanas 5 a 7 ataca exatamente isso.':'the ground. The plyometrics in weeks 5 to 7 attack exactly that.',
  'ppm nas rodagens, sem mudar o ritmo. O passo encurta sozinho.':'spm on easy runs, without changing pace. The stride shortens on its own.',
  '60 a 90 g de carboidrato por hora, começando na primeira hora.':'60 to 90 g of carbohydrate per hour, starting in the first hour.',
  'Backup completo: prova, treinos marcados, trocas e atividades.':'Full backup: race, marked workouts, swaps and activities.',
  'Nenhuma medida ainda. Elas chegam do app Sa\\u00fade do iPhone,':'No readings yet. They arrive from the iPhone Health app,',
  'O número maior tem que ser o de cima. Confira se não inverteu.':'The higher number has to be the top one. Check you did not swap them.',
  'Séries curtas e fortes para elevar o ritmo máximo sustentável.':'Short, hard sets to raise the maximum pace you can sustain.',
  'agachamento pesado sem trava de segurança fica para a Fit4Less':'heavy squats without a safety catch are left for Fit4Less',
  'ainda não começaram, e a semana em curso só entra na aderência':'have not started yet, and the week in progress only counts towards adherence',
  ', nas rodagens fáceis. Sobe quando você corre mais rápido com o':', on easy runs. It goes up when you run faster at the',
  'Trocar aqui vale só para este dia e não se perde ao recarregar.':'Swapping here applies to this day only and survives a reload.',
  'km em ritmo confortável, com 2 blocos de 5 min de cadência alta':'km at a comfortable pace, with 2 blocks of 5 min at high cadence',
  'Diferença entre os lados. Assimetria acima de 2% merece atenção.':'Difference between sides. Asymmetry above 2% deserves attention.',
  'Não consegui falar com o servidor — pode haver treino mais novo.':'I could not reach the server — there may be a newer workout.',
  'km em ritmo de rodagem. Resista à vontade de acelerar no começo.':'km at easy-run pace. Resist the urge to speed up at the start.',
  'km soltos e, no fim, 6 a 8 tiros de 10 a 12 s numa ladeira leve,':'km relaxed and, at the end, 6 to 8 reps of 10 to 12 s on a gentle hill,',
  'Encolher a próxima semana para o volume que você realmente cumpre':'Shrink next week to the volume you actually complete',
  'Ritmo constante, orientação a cada 8 braçadas se for água aberta.':'Steady pace, sighting every 8 strokes if it is open water.',
  '10×100 m com 20 s de pausa, ritmo constante do primeiro ao último.':'10×100 m with 20 s rest, steady pace from first to last.',
  'Aquele em que dá para conversar. Uso este se você não tiver marca.':'The one where you can talk. I use this if you have no benchmark.',
  'Não consegui falar com o servidor. Pode haver treino mais novo lá.':'I could not reach the server. There may be a newer workout there.',
  'Queda que se sustenta por vários dias costuma vir antes da fadiga.':'A drop that lasts several days usually comes before fatigue.',
  'Um plano que não se cumpre não treina ninguém — ele só gera culpa.':'A plan you do not complete trains nobody — it only creates guilt.',
  'Lento. Ensina o tronco a ficar quieto enquanto as pernas trabalham.':'Slow. It teaches the torso to stay quiet while the legs work.',
  'Volume aeróbico sem impacto. Constrói base e poupa as articulações.':'Aerobic volume without impact. Builds base and spares the joints.',
  'não uma meta — serve para ver o treino de hoje contra o seu normal.':'not a target — it is there to compare today\'s workout with your normal.',
  '. As primeiras não têm previsto porque o app ainda não guardava esse':'. The first ones have no plan because the app was not keeping that',
  'Assente no ritmo alvo e mantenha constante. Beba em todos os postos.':'Settle into target pace and hold it steady. Drink at every station.',
  'Prefiro te dizer isso a inventar um veredito em cima de três pontos.':'I would rather tell you that than invent a verdict from three points.',
  'os erros que mais aparecem e um vídeo de demonstração por exercício.':'the mistakes that come up most, and a demo video for each exercise.',
  'treino(s) mudam. Você pode desfazer cada um no próprio dia. Aplicar?':'workout(s) change. You can undo each one on its own day. Apply?',
  '8×100 m + 4×50 m de pernada. Respiração bilateral, a cada 3 braçadas.':'8×100 m + 4×50 m kick. Bilateral breathing, every 3 strokes.',
  'B · 3:55:00 (5:34/km) — classifica com folga sobre o corte histórico.':'B · 3:55:00 (5:34/km) — qualifies comfortably above the historical cut-off.',
  'Calculado como no TrainingPeaks: uma hora no pace de limiar vale 100.':'Calculated as in TrainingPeaks: one hour at threshold pace is worth 100.',
  'Resistência específica e ensaio de alimentação. Prioridade da semana.':'Specific endurance and a fuelling rehearsal. The week\'s priority.',
  'em hora: espere o relógio subir a atividade e puxe a tela para baixo.':'hour: wait for the watch to upload the activity and pull the screen down.',
  'A carga desta semana está coerente com o que seu corpo vem aguentando.':'This week\'s load is in line with what your body has been handling.',
  'Quanto menos o tronco sobe a cada passada, menos energia desperdiçada.':'The less the torso rises with each stride, the less energy is wasted.',
  'A conta usa as corridas que o Garmin registrou, não as etapas marcadas.':'The count uses the runs Garmin recorded, not the steps you ticked.',
  'Carga que você faria 7 vezes, não mais. Pare com 2 repetições de sobra.':'A load you could do 7 times, no more. Stop with 2 reps in reserve.',
  'O sync demorou mais que o esperado. Tente recarregar em alguns minutos.':'The sync took longer than expected. Try reloading in a few minutes.',
  '. Sessão-chave do ciclo: se esta sair inteira, 3:50 deixa de ser aposta.':'. Key session of the cycle: if this one comes out whole, 3:50 stops being a gamble.',
  '2 km aquecimento · 3×8 min a 5:00/km com 2 min de trote · 2 km soltando.':'2 km warm-up · 3×8 min at 5:00/km with 2 min jog · 2 km cool-down.',
  '2 km aquecimento · 4×5 min a 5:00/km com 2 min de trote · 2 km soltando.':'2 km warm-up · 4×5 min at 5:00/km with 2 min jog · 2 km cool-down.',
  '2 km aquecimento · 4×8 min a 5:00/km com 2 min de trote · 2 km soltando.':'2 km warm-up · 4×8 min at 5:00/km with 2 min jog · 2 km cool-down.',
  '2 km aquecimento · 6×3 min a 4:35/km com 2 min de trote · 1 km soltando.':'2 km warm-up · 6×3 min at 4:35/km with 2 min jog · 1 km cool-down.',
  'Cada botão abre o guia direto na etapa: passo a passo de cada movimento,':'Each button opens the guide straight at that step: every movement step by step,',
  'Empurre o quadril para trás, coluna neutra. Protege o joelho na descida.':'Push the hips back, spine neutral. It protects the knee on the way down.',
  'Os 2 primeiros km já são o aquecimento. Comece mais devagar do que quer.':'The first 2 km are the warm-up. Start slower than you want to.',
  '2 km aquecimento · 2×12 min a 5:00/km com 3 min de trote · 2 km soltando.':'2 km warm-up · 2×12 min at 5:00/km with 3 min jog · 2 km cool-down.',
  '2 km aquecimento · 3×10 min a 5:00/km com 2 min de trote · 1 km soltando.':'2 km warm-up · 3×10 min at 5:00/km with 2 min jog · 1 km cool-down.',
  '8 min bem leves. Se as pernas estiverem pesadas, comece caminhando 3 min.':'8 very easy min. If your legs feel heavy, start by walking 3 min.',
  'Técnica antes de volume. Na água, nadar melhor rende mais que nadar mais.':'Technique before volume. In the water, swimming better pays more than swimming more.',
  '10 min bem leves. Se as pernas estiverem pesadas, comece caminhando 3 min.':'10 very easy min. If your legs feel heavy, start by walking 3 min.',
  '12 min bem leves. Se as pernas estiverem pesadas, comece caminhando 3 min.':'12 very easy min. If your legs feel heavy, start by walking 3 min.',
  '40 a 50% — leia a tendência de semanas, nunca a fase de uma noite isolada.':'40 to 50% — read the trend over weeks, never one isolated night\'s stage.',
  'Enviado ao Hevy. Confira em um minuto. Isto nao marca o treino como feito.':'Sent to Hevy. Check in a minute. This does not mark the workout as done.',
  'do que o bloco pediu para esta semana. A aderência do ciclo aparece quando':'than the block asked for this week. Cycle adherence appears once',
  'ppm a passada está longa e o pé cai à frente do quadril — cada aterrissagem':'spm the stride is long and the foot lands ahead of the hip — every landing',
  'Blocos longos em ritmo firme, para segurar velocidade sem quebrar a braçada.':'Long blocks at a firm pace, to hold speed without breaking the stroke.',
  'minutos contínuos em esforço aeróbico. Deve dar para conversar o tempo todo.':'continuous minutes at aerobic effort. You should be able to talk the whole way.',
  'Está no nível que eu chamaria de excelente. Aqui não se persegue mais número:':'You are at what I would call an excellent level. Here you no longer chase numbers:',
  'Estímulo de potência aeróbica. Melhora a velocidade máxima que você sustenta.':'Aerobic power stimulus. It improves the top speed you can sustain.',
  'Pneus, freios, marchas e garrafas conferidos na véspera. Leve câmara e bomba.':'Tyres, brakes, gears and bottles checked the day before. Take a tube and a pump.',
  'Sem conexão com o servidor. Mostrando a última cópia guardada neste aparelho.':'No connection to the server. Showing the last copy saved on this device.',
  'Dias em destaque estão livres. Escolher um dia ocupado troca os dois de lugar.':'Highlighted days are free. Picking a busy day swaps the two around.',
  'porque duas semanas são pouca amostra para virar o plano de cabeça para baixo.':'because two weeks is too small a sample to turn the plan upside down.',
  'recente projeta, pela fórmula de Riegel. Serve para responder uma pergunta só:':'recent projects, using Riegel\'s formula. It answers one question only:',
  'Seus dados agora ficam protegidos por senha. Entre uma vez — o aparelho lembra.':'Your data is now protected by a password. Sign in once — the device remembers.',
  'Esses números vêm do relógio pelo Sync Garmin — sono, Body Battery, stress e HRV.':'These numbers come from the watch through Sync Garmin — sleep, Body Battery, stress and HRV.',
  'Nada grave, mas há detalhes que cobram juros lá na frente. Estão listados abaixo.':'Nothing serious, but there are details that charge interest later on. They are listed below.',
  'fazer menos barulho) e a pliometria das semanas 5 a 7. Compare sempre terreno com':'making less noise) and the plyometrics in weeks 5 to 7. Always compare like terrain with',
  'Cadência não tem número ideal fixo: ela sobe junto com a velocidade. Comparar seus':'Cadence has no fixed ideal number: it rises along with speed. Comparing your',
  'Estímulo cardiovascular sem impacto. Protege articulações e acelera a recuperação.':'Cardiovascular stimulus without impact. Protects the joints and speeds up recovery.',
  '4 × 60 m aumentando a velocidade até quase máxima, sem tensionar. Caminhe de volta.':'4 × 60 m building speed to near maximum, without tensing. Walk back.',
  '4×20 s: skipping alto, anfersen, dribles, passada saltada. Caminhe 40 s entre eles.':'4×20 s: high knees, butt kicks, quick feet, bounding. Walk 40 s between them.',
  'Eleva o ritmo que você aguenta por muito tempo. O treino que mais muda o resultado.':'It raises the pace you can hold for a long time. The workout that changes the result most.',
  'O volume parte da sua média real, não do que estava no papel, e sobe no máximo 10%.':'Volume starts from your real average, not from what was on paper, and rises by at most 10%.',
  'Se as pernas não respondem no dia forte, quase sempre a causa está no dia anterior.':'If your legs do not respond on the hard day, the cause is almost always the day before.',
  'km em ritmo de limiar. Termine com a sensação de que daria para mais um quilômetro.':'km at threshold pace. Finish feeling you could have done one more kilometre.',
  'E a concordância das fases medidas por relógio com um exame de sono fica em torno de':'And the agreement between watch-measured stages and a sleep study is around',
  'Faixas de refer\\u00eancia, para leitura \\u2014 quem interpreta \\u00e9 o m\\u00e9dico.':'Reference ranges, for reading — the doctor is the one who interprets them.',
  'Cai naturalmente conforme o pace melhora. Acompanhe a tendência, não o valor isolado.':'It falls naturally as pace improves. Follow the trend, not the isolated value.',
  'Melhor um volume menor cumprido inteiro do que um volume grande cumprido pela metade.':'Better a smaller volume completed in full than a big one completed by half.',
  'Oscilação dividida pelo comprimento da passada — a melhor medida isolada de economia.':'Oscillation divided by stride length — the best single measure of economy.',
  'Semana acima de 100% conta como 100%: correr a mais não compensa a semana que faltou.':'A week above 100% counts as 100%: running extra does not make up for the week you missed.',
  'Semana mais dura do ciclo: aqui só 6×100 m bem soltos, para tirar a rigidez do longo.':'Hardest week of the cycle: here just 6×100 m very relaxed, to take the stiffness out of the long run.',
  'aterrissar leve, com o pé debaixo do quadril, e evitar estender a perna para alcançar':'land lightly, with the foot under the hip, and avoid reaching out with the leg to meet',
  'km em ritmo de conversa. Se não consegue falar uma frase inteira, está rápido demais.':'km at conversational pace. If you cannot speak a full sentence, it is too fast.',
  'Pedindo ao GitHub para ler o Hevy. Leva cerca de 1 minuto — pode deixar a tela aberta.':'Asking GitHub to read Hevy. It takes about 1 minute — you can leave the screen open.',
  '6 km em ritmo de conversa. Se não consegue falar uma frase inteira, está rápido demais.':'6 km at conversational pace. If you cannot speak a full sentence, it is too fast.',
  'Quando eu achar que ele deve mudar, a proposta aparece aqui com o motivo e você decide.':'When I think it should change, the proposal appears here with the reason and you decide.',
  'km em ritmo aeróbico constante. Se tiver subida, mantenha o esforço — não a velocidade.':'km at a steady aerobic pace. If there is a climb, hold the effort — not the speed.',
  'Nada a mudar. O plano das próximas semanas continua adequado ao que você vem entregando.':'Nothing to change. The plan for the coming weeks still fits what you have been delivering.',
  'Rodagem fácil não serve: ela diz como você recupera, não a que ritmo você aguenta 42 km.':'An easy run will not do: it tells you how you recover, not the pace you can hold for 42 km.',
  'Duas ressalvas honestas: essas faixas são da fisiologia do sono, não um padrão do Garmin.':'Two honest caveats: these ranges come from sleep physiology, not from a Garmin standard.',
  'Volume sem impacto no dia seguinte ao longão. Acelera a recuperação em vez de atrapalhar.':'Volume without impact the day after the long run. It speeds recovery instead of hindering it.',
  '65 min em Z2, respirando pelo nariz. Se ofegar na subida, troque a marcha em vez de forçar.':'65 min in Z2, breathing through your nose. If you gasp on a climb, change gear instead of forcing.',
  '6×100 m + 4×50 m de pernada com prancha. A pernada solta o tornozelo, que corrida enrijece.':'6×100 m + 4×50 m kick with a board. Kicking loosens the ankle that running stiffens.',
  'Memória e regulação do humor. Cai quando você dorme pouco, porque vem mais no fim da noite.':'Memory and mood regulation. It drops when you sleep little, because it comes later in the night.',
  'Recupera músculo e consolida a adaptação ao treino. É a fase que o treino forte mais exige.':'It repairs muscle and locks in the adaptation to training. It is the stage hard training demands most.',
  'de trote leve entre as repetições. Todas no mesmo tempo — se a última cair muito, corte uma.':'of easy jog between the reps. All at the same time — if the last one drops off badly, cut one.',
  '. O maior do ciclo. Ensaio geral: mesma roupa, mesmo tênis, mesmo gel, mesma hora da largada.':'. The longest of the cycle. Full dress rehearsal: same clothes, same shoes, same gel, same start time.',
  'do que há três semanas. Isso costuma ser sono, calor ou carga acumulada — não perda de forma.':'than three weeks ago. That is usually sleep, heat or accumulated load — not a loss of fitness.',
  'km no ritmo mais leve que você tem. Se ficar em dúvida se está leve o bastante, diminua mais.':'km at the easiest pace you have. If you are unsure whether it is easy enough, go easier still.',
  'O ritmo que você sustenta por muito tempo. O treino que mais muda o resultado numa prova longa.':'The pace you can hold for a long time. The workout that changes the result most in a long race.',
  'Os saltos são amortecidos de propósito — o limiar anda no máximo 8 s/km e o volume 15% por vez,':'The jumps are damped on purpose — the threshold moves at most 8 s/km and the volume 15% at a time,',
  'aquele ritmo e vai sozinho. O custo aparece no limiar e no longo, que chegam com a perna gasta.':'that pace and goes off on its own. The cost shows up in the threshold and the long run, which arrive on worn legs.',
  'A linha caiu. Fadiga acumulada, calor ou noites ruins costumam explicar antes de perda de forma.':'The line fell. Accumulated fatigue, heat or bad nights usually explain it before a loss of fitness does.',
  'Ainda não chegou nenhuma corrida de hoje do Garmin. Se você já treinou, a sincronia roda de hora':'No run from today has arrived from Garmin yet. If you have already trained, the sync runs every',
  'As trocas precisam estar em ordem: troca 1 menor que troca 2, e as duas menores que a distância.':'The switches must be in order: switch 1 smaller than switch 2, and both smaller than the distance.',
  'em ritmo tranquilo, focando em braçada longa. Conte as braçadas por piscina e tente reduzir uma.':'at an easy pace, focusing on a long stroke. Count your strokes per length and try to take one off.',
  'Passos curtos e rápidos, braços ativos, olhar 10 metros à frente. Não jogue o tronco para frente.':'Short, quick steps, active arms, eyes 10 metres ahead. Do not throw your torso forward.',
  'Troque o calçado em menos de 90 segundos. Treine isso de verdade — na prova custa tempo e cabeça.':'Change your shoes in under 90 seconds. Practise it for real — on race day it costs time and focus.',
  'Você cancelou o treino que o plano tinha posto aqui. Pode restaurá-lo, ou incluir outro no lugar.':'You cancelled the workout the plan had put here. You can restore it, or add another in its place.',
  '5 min de trote muito leve, depois 5 min de mobilidade: panturrilha, posterior, quadril e tornozelo.':'5 min of very easy jog, then 5 min of mobility: calves, hamstrings, hips and ankles.',
  'Há dias de plano na janela, mas nenhum foi avaliado — as atividades do Garmin não casaram com eles.':'There are plan days in the window, but none was assessed — the Garmin activities did not match them.',
  'Longo com a perna já cansada do dia anterior. É o treino que mais se parece com o fim de uma ultra.':'A long run on legs already tired from the day before. It is the workout most like the end of an ultra.',
  'nas duas semanas. Amostra pequena demais para recalibrar com honestidade — o plano segue como está.':'over the two weeks. Too small a sample to recalibrate honestly — the plan stays as it is.',
  'Solto. Treine o abastecimento: um gel aos 40 min e a cada 40 min depois. Use o que vai usar na prova.':'Relaxed. Practise your fuelling: a gel at 40 min and every 40 min after. Use what you will use on race day.',
  'A cada semana, seu melhor esforço das seis anteriores projetado para 42,195 km pela fórmula de Riegel.':'Each week, your best effort from the previous six projected to 42.195 km using Riegel\'s formula.',
  'Despertares curtos são normais. Muito acima de 5% costuma indicar álcool, calor ou treino tarde demais.':'Short awakenings are normal. Well above 5% usually points to alcohol, heat or training too late.',
  ', mais devagar que o alvo, de propósito. Depois assente em 5:27. Se aos 32 km ainda estiver bem, aperte.':', slower than target, on purpose. Then settle into 5:27. If you are still fine at 32 km, push.',
  'Alongamento parado fica para o fim — feito agora, ele reduz a força que você vai precisar daqui a pouco.':'Static stretching is left for the end — done now, it reduces the strength you will need in a moment.',
  '4 séries de 20 segundos: skipping alto, anfersen, dribles e passada saltada. Caminhe 40 segundos entre elas.':'4 sets of 20 seconds: high knees, butt kicks, quick feet and bounding. Walk 40 seconds between them.',
  'minutos contínuos em ritmo "confortavelmente difícil". Dá para falar 3 ou 4 palavras, não uma frase inteira.':'continuous minutes at a "comfortably hard" pace. You can say 3 or 4 words, not a full sentence.',
  '× 100 m em ritmo forte, com 20 segundos de descanso. Todas no mesmo tempo — anote e compare na próxima semana.':'× 100 m at a hard pace, with 20 seconds rest. All at the same time — write it down and compare next week.',
  '75 min em Z2. O maior giro do ciclo — no dia seguinte vem o longo com 12 km em ritmo, então nada de forçar hoje.':'75 min in Z2. The biggest ride of the cycle — tomorrow comes the long run with 12 km at pace, so nothing hard today.',
  'balanço lateral, círculo de quadril, agachamento livre, afundo com rotação de tronco e elevação de joelho andando.':'lateral swings, hip circles, bodyweight squats, lunge with torso rotation and walking knee raises.',
  'sem parar, em ritmo constante. Se for prova em águas abertas, levante a cabeça para se orientar a cada 8 braçadas.':'without stopping, at a steady pace. If it is an open-water race, lift your head to sight every 8 strokes.',
  'Fartlek treina a leitura do próprio esforço. Você vai precisar disso quando o relógio falhar ou o percurso enganar.':'Fartlek trains you to read your own effort. You will need that when the watch fails or the course deceives you.',
  'km contínuos em ritmo confortável. Você deve conseguir falar frases inteiras sem ofegar — se não conseguir, diminua.':'continuous km at a comfortable pace. You should be able to speak full sentences without gasping — if you cannot, ease off.',
  '4 séries de 20 segundos, caminhando 40 segundos entre elas: skipping alto, anfersen, dribles baixos e passada saltada.':'4 sets of 20 seconds, walking 40 seconds between them: high knees, butt kicks, low quick feet and bounding.',
  '6 × 50 m: um braço só, ponta dos dedos na água, pegada com antebraço alto, respiração bilateral. 20 segundos de descanso.':'6 × 50 m: single arm, fingertip drag, high-elbow catch, bilateral breathing. 20 seconds rest.',
  'Depois de treinar, o Atalho do iPhone manda a sessão de volta para cá pelo app Saúde — ela aparece na aba Treinos sozinha.':'After you train, the iPhone Shortcut sends the session back here through the Health app — it shows up on the Workouts tab by itself.',
  '4 × 20 segundos no ritmo que pretende fazer na prova, com 60 segundos de caminhada entre elas. Termine querendo fazer mais.':'4 × 20 seconds at the pace you intend to race, with 60 seconds of walking between them. Finish wanting to do more.',
  'Este aparelho ainda não está conectado ao GitHub. Cole o token na janela que abriu e depois toque em “Já registrei” de novo.':'This device is not connected to GitHub yet. Paste the token in the window that opened and then tap “Logged it in Hevy” again.',
  'Suba devagar. Enjoo ou vontade de banheiro querem dizer que você passou do ponto — volte um degrau e fique nele mais duas semanas.':'Build up slowly. Nausea or an urgent trip to the toilet mean you went past the point — drop back a step and stay there two more weeks.',
  'Estas respostas passam a valer para todos os treinos que o app montar daqui para frente. Dá para refazer quando quiser, na aba Dados.':'These answers apply to every workout the app builds from now on. You can redo them whenever you like, on the Data tab.',
  'km. Dá para chegar lá, mas não em um ciclo só. Um alvo intermediário primeiro costuma ser o caminho mais curto — e o que menos machuca.':'km. You can get there, but not in one cycle. An intermediate target first is usually the shortest route — and the one that hurts least.',
  'segundos subindo forte numa ladeira de inclinação média. Volte trotando ou caminhando, sem pressa — a recuperação precisa ser completa.':'seconds climbing hard on a moderate hill. Come back jogging or walking, no rush — the recovery has to be complete.',
  '2 km aquecimento · 8×60 s subindo forte, descendo trotando · 2 km soltando. Força de perna sem academia e sem o impacto do tiro no plano.':'2 km warm-up · 8×60 s climbing hard, jogging back down · 2 km cool-down. Leg strength without the gym and without the impact of flat reps.',
  'Acorde 3 h antes. Café da manhã testado nos longos, nada novo. Chegue com 1 h de folga: são 7h da manhã e vai estar frio em Charlottetown.':'Wake up 3 h before. Breakfast tested on your long runs, nothing new. Arrive with 1 h to spare: it is 7 a.m. and it will be cold in Charlottetown.',
  'O ritmo que você sustenta por cerca de uma hora. É o mais importante daqui: dele saem todos os outros. Hoje sua rodagem leve está saindo em':'The pace you can hold for about an hour. It is the most important one here: every other pace comes from it. Right now your easy runs are coming out at',
  'Se o corpo pedir descanso, descanse. Isto está aqui para soltar quadril e tornozelo no dia seguinte ao longo, com zero impacto — não para somar treino.':'If your body asks for rest, rest. This is here to loosen hips and ankles the day after the long run, with zero impact — not to add training.',
  'Use exatamente o que vai usar na prova: mesma comida no café, mesmo horário, mesma roupa, mesmos géis. O que der errado hoje você ainda tem três semanas para arrumar.':'Use exactly what you will use on race day: same breakfast, same time, same clothes, same gels. Whatever goes wrong today you still have three weeks to fix.',
  'em relação à cinza, você ganhou base aeróbica: mais velocidade com menos esforço cardíaco. É mais confiável que o pace sozinho, que depende do dia, do terreno e do calor.':'compared with the grey one, you have gained aerobic base: more speed at less cardiac effort. It is more reliable than pace alone, which depends on the day, the terrain and the heat.',
  'Correr encurta e enrijece flexor de quadril e panturrilha. Nadar move as mesmas articulações em amplitude total, sem peso do corpo. Aos 64 isso vale mais que um dia parado.':'Running shortens and stiffens the hip flexors and calves. Swimming moves the same joints through full range, with no body weight. At 64 that is worth more than a day off.',
  'Enquanto você marcar dor ao correr ou lesão que ainda incomoda, os treinos fortes viram rodagem leve e o volume fica reduzido. Refaça o questionário quando melhorar e a intensidade volta sozinha.':'While you report pain when running or an injury that still bothers you, hard workouts become easy runs and the volume stays reduced. Redo the questionnaire when it improves and the intensity comes back on its own.',
  'As perguntas de saúde seguem o PAR-Q, o questionário padrão de prontidão para atividade física. O app não diagnostica nada — ele só avisa quando vale conversar com um médico antes de subir a carga.':'The health questions follow the PAR-Q, the standard physical activity readiness questionnaire. The app diagnoses nothing — it only flags when it is worth talking to a doctor before increasing the load.',
  'Você marcou pelo menos uma situação da triagem de saúde. Isso não quer dizer que você não possa correr — quer dizer que a decisão de subir volume ou intensidade não deveria ser tomada só por um aplicativo. Enquanto isso, o plano fica mais conservador e sem treino forte.':'You ticked at least one item on the health screening. That does not mean you cannot run — it means the decision to raise volume or intensity should not be taken by an app alone. In the meantime, the plan stays more conservative and without hard workouts.',
  };
  for(var __k5 in EN_FONTE) if(Object.prototype.hasOwnProperty.call(EN_FONTE, __k5)) EN_FRASES[__k5] = EN_FONTE[__k5];


  /* ── sexta leva: as frases que as regras genéricas estavam
     traduzindo pela metade. Agora cada uma tem a frase inteira, e a
     regra que virava "não é" em "não is" saiu de cena.              */
  var EN_FONTE2 = {
  'dias é':'days is',
  'Execução:':'How to do it:',
  'sem dados':'no data',
  ...
  '· sem treino':'· no workout',
  'Só as últimas':'Only the last',
  'sem treino em':'no workout on',
  'só no relógio':'on the watch only',
  'Mais de 5 anos':'More than 5 years',
  'Para que serve':'What it is for',
  'Treino no Hevy':'Workout in Hevy',
  'semana. As com':'week. The ones marked',
  '— por causa de':'— because of',
  'Dias de treino:':'Training days:',
  'para o seu pace':'for your pace',
  'sem treino hoje':'no workout today',
  'Menos de 6 meses':'Less than 6 months',
  'M\\u00e9dia 7 dias':'7-day average',
  'Nenhuma atividade':'No activity',
  'dias para a prova':'days to race day',
  'km e o objetivo é':'km and the goal is',
  'M\\u00e9dia 30 dias':'30-day average',
  'só o peso do corpo':'bodyweight only',
  'Por DISTÂNCIA. Alvo':'By DISTANCE. Target',
  'Sem registros desde':'No records since',
  ', porque aqui não há':', because there is no',
  'sem carga registrada':'no load logged',
  'Como fazer cada etapa':'How to do each step',
  'Elas não voltam mais.':'They do not come back.',
  'Força — Máxima (pico)':'Strength — Max (peak)',
  'Sua maior distância é':'Your longest distance is',
  '· hoje o plano propõe':'· today the plan proposes',
  '— a que você informou':'— the one you entered',
  '— estimada pela idade':'— estimated from your age',
  'Como está no seu Hevy.':'As it is in your Hevy.',
  ...
  'Por que sem cronômetro':'Why with no stopwatch',
  'Pratica outro esporte?':'Do you do another sport?',
  '· o próximo do plano é':'· the next one in the plan is',
  '— (ainda não composto)':'— (not built yet)',
  ': faça antes de apertar':': do this before pressing',
  'Não há plano registrado':'No plan on record',
  'Suas próximas 2 semanas':'Your next 2 weeks',
  '— relendo as atividades':'— re-reading the activities',
  'Nenhum objetivo definido':'No goal set',
  'Nenhum treino neste mês.':'No workouts this month.',
  'core para o fim da prova':'core for the end of the race',
  'desde o começo do ciclo.':'since the cycle began.',
  'sem semanas para mostrar':'no weeks to show',
  'sessão sem quilometragem':'session with no distance',
  'Marcar como feito em casa':'Mark as done at home',
  'Natação — técnica e série':'Swim — technique and main set',
  'Sem dados para recalibrar':'No data to recalibrate',
  'Treino do plano cancelado':'Plan workout cancelled',
  'km, seu maior nas últimas':'km, your longest in the last',
  'no chão, sem banco romano':'on the floor, no roman chair',
  'A projeção já cabe no alvo':'The projection already fits the target',
  'Sem atividades no servidor':'No activities on the server',
  'Sua alavanca é a cadência.':'Your lever is cadence.',
  'menos de um ano de corrida':'less than a year of running',
  'sem treino de corrida hoje':'no run scheduled today',
  'Encurtar o próximo longo de':'Shorten the next long run from',
  'Restaurar o treino do plano':'Restore the plan\'s workout',
  'contra as 3 semanas antes ·':'against the 3 weeks before ·',
  'menos de 6 meses de corrida':'less than 6 months of running',
  'para o KPI ter o que medir.':'so the KPI has something to measure.',
  'Compor as próximas 2 semanas':'Build the next 2 weeks',
  'O objetivo é circular sangue':'The goal is to circulate blood',
  'Sem pace alvo para comparar.':'No target pace to compare with.',
  'dessas é um freio. Suba para':'of those is a brake. Raise it to',
  'nenhuma corrida nos próximos':'no run in the next',
  'o próximo longo não passa de':'the next long run goes no further than',
  'sim — nunca conta como feito':'yes — never counts as done',
  'E-mail ou senha não conferem.':'E-mail or password do not match.',
  'Leve, sem carga, 30 a 40 min.':'Easy, no load, 30 to 40 min.',
  'Tracejada: previsto que falta':'Dashed: still to do',
  '— calculado a partir dos seus':'— calculated from your',
  '. No período, o pico médio foi':'. Over the range, the average peak was',
  'Distância contínua, sem parar.':'Continuous distance, no stopping.',
  'atenção: FC máxima configurada':'note: configured max HR',
  'guardadas. A mais recente é de':'stored. The most recent is from',
  'sem nutrição marcada neste dia':'no fuelling marked on this day',
  '— estimado pela sua rodagem de':'— estimated from your easy run of',
  '4×100 m. Só para mexer o corpo.':'4×100 m. Just to move the body.',
  'km que o plano pediu até hoje ·':'km the plan has asked for so far ·',
  'para este dia entrar na janela.':'for this day to fall in the window.',
  'Nenhum dado de saúde no período.':'No health data in this range.',
  'Nenhuma prova com esses filtros.':'No race matches these filters.',
  'O plano não tem nenhum dia entre':'The plan has no day between',
  'Teve lesão nos últimos 12 meses?':'Have you been injured in the last 12 months?',
  'Treino de casa: faça sem o Hevy.':'Home workout: do it without Hevy.',
  'o último que chegou do Hevy é de':'the last one that arrived from Hevy is from',
  'O que muda nas próximas 2 semanas':'What changes in the next 2 weeks',
  'Sem impacto, soma volume aeróbico':'No impact, adds aerobic volume',
  'Sessão sem detalhamento definido.':'Session with no detail defined.',
  'no último treino · sua mediana em':'on the last workout · your median over',
  '. No Hevy, confira se o treino foi':'. In Hevy, check whether the workout was',
  '. O próximo bloco será composto em':'. The next block will be built on',
  'Faz musculação ou treino de força?':'Do you do weights or strength training?',
  'Sem leituras de stress no período.':'No stress readings in this range.',
  'Só se você marcou às vezes ou sim.':'Only if you answered sometimes or yes.',
  'km, 15% acima do seu maior recente':'km, 15% above your recent longest',
  'nenhum treino chegou do Hevy ainda':'no workout has arrived from Hevy yet',
  '15 minutos leves com 2 acelerações.':'15 easy minutes with 2 strides.',
  'Para a maratona, o ideal é chegar a':'For the marathon, the ideal is to reach',
  'Sente dor ou desconforto ao correr?':'Do you feel pain or discomfort when running?',
  'corrida é exercício de uma perna só':'running is a single-leg exercise',
  'para esta semana — nada a comparar.':'for this week — nothing to compare.',
  '· no Hevy: ⋯ › Substituir exercício':'· in Hevy: ⋯ › Replace exercise',
  'Dor não é para ser treinada por cima':'Pain is not something to train through',
  'Nenhuma atividade válida encontrada.':'No valid activity found.',
  'Pendente. Ao terminar, toque abaixo.':'Pending. When you finish, tap below.',
  'para investigar tontura ao levantar.':'to investigate dizziness on standing.',
  'Nenhum treino do plano avaliado ainda':'No plan workout assessed yet',
  ', porque é o que o iPhone está usando.':', because that is what the iPhone is using.',
  'Giro leve, 30 a 40 minutos, sem carga.':'Easy spin, 30 to 40 minutes, no load.',
  'Qual é o seu objetivo principal agora?':'What is your main goal right now?',
  'Semana leve — é aqui que o ganho gruda':'Easy week — this is where the gains stick',
  'min de trote leve. Nao pare de correr.':'min of easy jog. Do not stop running.',
  'O Hevy entrega aqui os últimos 6 meses.':'Hevy delivers the last 6 months here.',
  'Blocos firmes e constantes, sem oscilar.':'Firm, steady blocks, no surging.',
  'Não havia treino previsto para este dia,':'There was no workout planned for this day,',
  'sem corridas de 8 km ou mais nos últimos':'no runs of 8 km or more in the last',
  'A média das últimas quatro semanas serve.':'The average of the last four weeks will do.',
  'Quantas vezes por semana você corre hoje?':'How many times a week do you run now?',
  'Sem data escolhida, o quadro não aparece.':'With no date chosen, the panel does not appear.',
  'km nos últimos 7 dias contra uma média de':'km in the last 7 days against an average of',
  '— seu histórico comporta o ciclo completo':'— your history supports the full cycle',
  'Descarga — resista à vontade de fazer mais':'Unload — resist the urge to do more',
  'Impacto alto, estímulo específico da prova':'High impact, race-specific stimulus',
  'Nenhum treino de academia neste intervalo.':'No gym workouts in this range.',
  'O pedaço no alto de cada barra é a soneca:':'The piece at the top of each bar is the nap:',
  'O robô leu o Hevy, mas lá não há treino em':'The robot read Hevy, but there is no workout there on',
  'Por TEMPO, trote leve. Não pare de correr.':'By TIME, easy jog. Do not stop running.',
  '— os trechos fortes podem ter ficado leves':'— the hard stretches may have come out easy',
  'Previne lesão e melhora economia de corrida':'Prevents injury and improves running economy',
  'Sync disparado. Recarreguei após 2 minutos.':'Sync triggered. I reloaded after 2 minutes.',
  'Tem tontura, desmaio ou perda de equilíbrio':'Do you have dizziness, fainting or loss of balance',
  'Você não completou nenhum longo nos últimos':'You did not complete a single long run in the last',
  'sem alvo fixo: o que importa é a progressão':'no fixed target: what matters is the progression',
  'A periodização não tem rotina para este dia.':'The periodisation has no routine for this day.',
  'Voltar à rotina depois da 25k de 2 de agosto':'Back into routine after the 25k on 2 August',
  'km de treino no ciclo, mais os 42,2 da prova':'km of training in the cycle, plus the 42.2 of the race',
  '40 min bem leves. Última bike antes da prova.':'40 very easy min. Last ride before the race.',
  'Quantas horas você dorme por noite, em média?':'How many hours do you sleep per night, on average?',
  'a partir de hoje, que é o preparo completo de':'from today, which is the full build-up of',
  'banco com suportes, sem trava: carga moderada':'bench with supports, no safety catch: moderate load',
  'sempre é amostra pequena, não condicionamento':'always a small sample, not fitness',
  'Dias em destaque ainda não têm segundo treino.':'Highlighted days do not have a second workout yet.',
  'Fique atrás no pelotão, gaste menos. Não puxe.':'Sit in the pack, spend less. Do not pull.',
  'Volume baixo: a corrida é a prioridade do dia.':'Low volume: running is the priority of the day.',
  'km bem leves. Nada de testar ritmo na véspera.':'very easy km. No pace testing the day before.',
  ', indo do fundo ao pico de energia. Média: pico':', running from the low to the peak of energy. Average: peak',
  'Toma medicação contínua para coração ou pressão':'Do you take ongoing medication for heart or blood pressure',
  'ppm já é adequada — forçar mais deixa a passada':'spm is already adequate — pushing higher makes the stride',
  'nos suportes, sem trava: carga que você controla':'on the supports, no catch: a load you control',
  '· a duração é convertida para manter o estímulo.':'· the duration is converted to keep the stimulus.',
  '· copie e cole no MOTRA para programar a sessão.':'· copy and paste into MOTRA to program the session.',
  '— passos mais curtos e frequentes, sem acelerar.':'— shorter, more frequent steps, without speeding up.',
  '20 minutos girando leve com 3 acelerações curtas.':'20 minutes spinning easy with 3 short accelerations.',
  'Aqui é onde o treino aparece. Se sobrou, acelere.':'This is where the training shows. If you have anything left, push.',
  'Só este treino muda de dia. O treino principal de':'Only this workout moves day. The main workout of',
  'bpm. A sua máxima real é provavelmente mais alta,':'bpm. Your real maximum is probably higher,',
  'Últimos 200 m aumentando a frequência de braçada.':'Last 200 m increasing your stroke rate.',
  '— o mesmo número do cartão acima. Os alongamentos':'— the same number as the card above. The stretches',
  'A faixa verde clara é onde a recuperação acontece.':'The light green band is where recovery happens.',
  'Corrida é exercício de uma perna só. Treine assim.':'Running is a single-leg exercise. Train it that way.',
  'carga alta, poucas repetições: economia de corrida':'heavy load, few reps: running economy',
  'e grande o bastante para contar até 18 de outubro.':'and big enough to count towards October 18.',
  'Descanso hoje — a fase atual não prevê sessão nesta':'Rest today — the current phase has no session on this',
  'num longão, pelo menos três semanas antes da prova.':'on a long run, at least three weeks before the race.',
  'ppm a cadência não é o problema. Olhe para o toque:':'spm cadence is not the problem. Look at your foot strike:',
  'referência para adultos. Média do período: profundo':'reference for adults. Range average: deep',
  '. O número no alto é a última leitura: pico e fundo.':'. The number at the top is the last reading: peak and low.',
  'Adaptada para os aparelhos de casa. Faça sem o Hevy.':'Adapted to your home equipment. Do it without Hevy.',
  'dias tiveram uma atividade correspondente no Garmin.':'days had a matching activity in Garmin.',
  'o previsto não traz ritmo, então não há TSS previsto':'the plan carries no pace, so there is no planned TSS',
  'Do seu Omron. A posição do corpo é o que mais importa':'From your Omron. Body position is what matters most',
  'O número a acompanhar não é o valor de hoje, é se ele':'The number to watch is not today\'s value, it is whether it',
  'Os indicadores aparecem na primeira semana de treino.':'The indicators appear in the first week of training.',
  'Nenhuma corrida com o dado ainda. Ele precisa da cinta':'No run with that data yet. It needs the strap',
  'dias marcados. O plano distribui os treinos só nestes.':'days selected. The plan spreads workouts across these only.',
  'tranquilos, sem série. Só manter o contato com a água.':'easy, no main set. Just keep the feel for the water.',
  'Ainda não há semana com plano registrado para comparar.':'There is no week with a plan on record to compare yet.',
  'Este dia ainda não tem treino porque o plano só vai até':'This day has no workout yet because the plan only runs to',
  'pelo Atalho \\u2014 o mesmo caminho dos treinos manuais.':'through the Shortcut — the same path as manual workouts.',
  'Elas param de voltar. Seus treinos de corrida não mudam.':'They stop coming back. Your running workouts do not change.',
  'Estável. Em semana de carga alta, manter já é bom sinal.':'Steady. In a heavy week, holding is already a good sign.',
  'Quantos treinos de corrida que o plano pediu nos últimos':'How many of the running workouts the plan asked for in the last',
  'A semana em curso é medida só pelos dias que já passaram.':'The week in progress is measured only by the days already gone.',
  'uma atividade só — que é exatamente o que o plano espera.':'one single activity — which is exactly what the plan expects.',
  'Carga subindo rápido. Vale segurar o próximo treino forte.':'Load rising fast. Worth holding back the next hard workout.',
  'Só dentro de tiros de 2 a 4 min. Nunca como bloco contínuo':'Only inside 2 to 4 min reps. Never as a continuous block',
  'realmente treinou — planejar mais longe que isso é ficção.':'you actually trained — planning further ahead than that is fiction.',
  'O que está na tela é exemplo. Toque em Sync Garmin no topo.':'What is on the screen is sample data. Tap Sync Garmin at the top.',
  'Ainda não há esforço recente forte o bastante para projetar.':'There is no recent effort hard enough to project from yet.',
  'Guarde algo para os últimos 20 km — é onde a prova se decide.':'Save something for the last 20 km — that is where the race is decided.',
  'O longo fica intacto: ele é o treino que constrói a maratona.':'The long run stays untouched: it is the workout that builds the marathon.',
  'Um único Workout, do começo ao fim. O relógio grava tudo como':'A single Workout, start to finish. The watch records it all as',
  'km. Contam para a carga, mesmo não contando para a aderência.':'km. They count towards load, even though they do not count towards adherence.',
  'Cada ponto é uma corrida. Se a nuvem verde está deslocada para':'Each dot is a run. If the green cloud is shifted',
  'Nenhuma medida ainda. Elas chegam do app Sa\\u00fade do iPhone,':'No readings yet. They arrive from the iPhone Health app,',
  '. Ele é reescrito de duas em duas semanas, a partir do que você':'. It is rewritten every two weeks, from what you',
  'Abaixo de 80% em duas semanas seguidas, é hora de rever o alvo.':'Below 80% two weeks running, it is time to review the target.',
  'O sentado pega o sóleo, que é onde o aquiles do veterano rompe.':'The seated one hits the soleus, which is where the veteran\'s achilles tears.',
  'OK volta a criar a força como segundo treino nos dias do plano.':'OK goes back to creating strength as a second workout on the plan\'s days.',
  '3 horas antes: algo leve e conhecido. Nada novo no dia da prova.':'3 hours before: something light and familiar. Nothing new on race day.',
  'O treino de hoje já é forte — some só se estiver se sentindo bem':'Today\'s workout is already hard — add it only if you are feeling good',
  '10 minutos girando leve, cadência entre 85 e 95 rpm, carga baixa.':'10 minutes spinning easy, cadence between 85 and 95 rpm, low load.',
  'Esta comparação precisa de 8 corridas com frequência cardíaca. Em':'This comparison needs 8 runs with heart rate. In',
  'Uma vez é cansaço ou calor. Repetido, é sinal de carga acumulada.':'Once is tiredness or heat. Repeated, it is a sign of accumulated load.',
  'o start, com o relógio parado. Eles não são quilômetro de treino.':'the start, with the watch stopped. They are not training kilometres.',
  'Aquele em que dá para conversar. Uso este se você não tiver marca.':'The one where you can talk. I use this if you have no benchmark.',
  'Um plano que não se cumpre não treina ninguém — ele só gera culpa.':'A plan you do not complete trains nobody — it only creates guilt.',
  '. As primeiras não têm previsto porque o app ainda não guardava esse':'. The first ones have no plan because the app was not keeping that',
  'Dia da prova. Confie no que foi treinado e execute o plano de ritmo.':'Race day. Trust what you trained and execute the pacing plan.',
  'só significam alguma coisa com duas ou três semanas de plano rodado.':'only mean something after two or three weeks of the plan running.',
  '15 minutos progressivos, terminando com 3 acelerações de 30 segundos.':'15 progressive minutes, finishing with 3 strides of 30 seconds.',
  'Calculado como no TrainingPeaks: uma hora no pace de limiar vale 100.':'Calculated as in TrainingPeaks: one hour at threshold pace is worth 100.',
  'Pendente. Registre o treino no Hevy e depois toque em “Já registrei”.':'Pending. Log the workout in Hevy and then tap “Logged it in Hevy”.',
  'Qualquer um dos três é uma primeira maratona bem corrida aos 64 anos.':'Any of the three is a first marathon well run at 64.',
  'positivo é bom · 3 a 8% em 3 semanas é ganho real · acima de 15% quase':'positive is good · 3 to 8% in 3 weeks is real gain · above 15% is almost',
  '2 km aquecimento · 2×5 km a 5:27/km com 3 min de trote · 1 km soltando.':'2 km warm-up · 2×5 km at 5:27/km with 3 min jog · 1 km cool-down.',
  '2 km aquecimento · 3×3 km a 5:27/km com 2 min de trote · 1 km soltando.':'2 km warm-up · 3×3 km at 5:27/km with 2 min jog · 1 km cool-down.',
  '2 km aquecimento · 3×5 km a 5:27/km com 3 min de trote · 1 km soltando.':'2 km warm-up · 3×5 km at 5:27/km with 3 min jog · 1 km cool-down.',
  '8 km fáceis. Não invente nada: o longo de domingo é o treino da semana.':'8 easy km. Do not invent anything: Sunday\'s long run is the week\'s workout.',
  'Carga que você faria 7 vezes, não mais. Pare com 2 repetições de sobra.':'A load you could do 7 times, no more. Stop with 2 reps in reserve.',
  'Distância contínua. Prepara cabeça e ombros para o tempo real da prova.':'Continuous distance. It prepares head and shoulders for the real race time.',
  'Não registrado. Se treinou, registre no Hevy e toque em “Já registrei”.':'Not logged. If you trained, log it in Hevy and tap “Logged it in Hevy”.',
  '. Peso total = carga × repetições de todas as séries registradas no Hevy.':'. Total weight = load × reps across every set logged in Hevy.',
  '3 × 30 segundos no ritmo de prova, com 2 minutos de giro leve entre eles.':'3 × 30 seconds at race pace, with 2 minutes of easy spinning between them.',
  'o que conta para outubro é o que suas pernas fizeram nas últimas semanas.':'what counts for October is what your legs did in the last few weeks.',
  'semanas — o plano entra direto na fase final, com foco em chegar inteiro.':'weeks — the plan goes straight into the final phase, focused on arriving in one piece.',
  'Enviado ao Hevy. Confira em um minuto. Isto nao marca o treino como feito.':'Sent to Hevy. Check in a minute. This does not mark the workout as done.',
  'Não é para ganhar músculo: deixa o tendão devolver energia a cada passada.':'It is not to gain muscle: it lets the tendon return energy on every stride.',
  '2 km soltos · 3 km a 5:27/km · 3 km soltos. Só para o corpo lembrar o ritmo.':'2 km relaxed · 3 km at 5:27/km · 3 km relaxed. Just so the body remembers the pace.',
  'Nenhuma medida traz a posi\\u00e7\\u00e3o do corpo. Para investigar tontura ao':'No reading carries body position. To investigate dizziness on',
  'Só aparelhos que você não tem em casa. Esta rotina ainda não existe no Hevy:':'Only machines you don\'t have at home. This routine isn\'t in Hevy yet:',
  'de sono em 24 h. As porcentagens acima são só da noite, que é onde o relógio':'of sleep in 24 h. The percentages above are from the night only, which is where the watch',
  'dias. A linha de baixo separa o que saiu do plano do que você fez por fora —':'days. The bottom line separates what came from the plan from what you did outside it —',
  'A maior parte da noite. Não é sono ruim: é o tecido que liga as outras fases.':'Most of the night. It is not bad sleep: it is the tissue that links the other stages.',
  'Caminhe 10 minutos, hidrate e coma carboidrato com proteína na primeira hora.':'Walk 10 minutes, hydrate and eat carbohydrate with protein in the first hour.',
  'Está no nível que eu chamaria de excelente. Aqui não se persegue mais número:':'You are at what I would call an excellent level. Here you no longer chase numbers:',
  'O próximo limiar longo ou um trecho em ritmo dentro do longão já dá o número.':'The next long threshold, or a stretch at pace inside the long run, will give the number.',
  'Para buscar o treino no Hevy, o app precisa do mesmo token do GitHub usado no':'To fetch the workout from Hevy, the app needs the same GitHub token used in',
  'Sem conexão com o servidor. Mostrando a última cópia guardada neste aparelho.':'No connection to the server. Showing the last copy saved on this device.',
  '. Correr no ritmo com a perna cansada é o treino mais parecido com a maratona.':'. Running at pace on tired legs is the workout most like the marathon.',
  'km um pouco mais fortes, sem chegar a ficar difícil. Simula a fadiga da prova.':'km a little harder, without becoming difficult. It simulates race fatigue.',
  'Não é treino, é recuperação ativa. Correr forte aqui rouba do treino de amanhã.':'It is not training, it is active recovery. Running hard here steals from tomorrow\'s workout.',
  'descendo caminhando com recuperação completa. São curtos demais para cansar — é':'walking back down with full recovery. They are too short to tire you — it is',
  '5 minutos sem forçar amplitude. Véspera de prova não é dia de descobrir dor nova.':'5 minutes without forcing the range. The day before a race is no day to discover new pain.',
  '8 minutos bem leves. Se as pernas estiverem pesadas, comece caminhando 3 minutos.':'8 very easy minutes. If your legs feel heavy, start by walking 3 minutes.',
  'Entram corridas de 8 km ou mais em ritmo de esforço — uma rodagem leve não conta.':'Runs of 8 km or more at effort pace count — an easy run does not.',
  'levantar, o que vale \\u00e9 medir deitado e depois em p\\u00e9, com 1 e 3 minutos.':'standing, what counts is measuring lying down and then standing, at 1 and 3 minutes.',
  'Cadência não tem número ideal fixo: ela sobe junto com a velocidade. Comparar seus':'Cadence has no fixed ideal number: it rises along with speed. Comparing your',
  '50 min soltos, plano. Amanhã é o longo de 32 km: as pernas precisam chegar frescas.':'50 relaxed min, flat. Tomorrow is the 32 km long run: your legs need to arrive fresh.',
  'O treino que mais se aproxima da exigência da prova. Prioridade absoluta da semana.':'The workout closest to race demands. The week\'s absolute priority.',
  'Se as pernas não respondem no dia forte, quase sempre a causa está no dia anterior.':'If your legs do not respond on the hard day, the cause is almost always the day before.',
  'minutos em ritmo bem leve. Comece andando 3 minutos se as pernas estiverem pesadas.':'minutes at a very easy pace. Start by walking 3 minutes if your legs feel heavy.',
  'frenética e ganha pouco. Daqui em diante o ganho é lento: toque leve (corra tentando':'frantic and gains little. From here the gain is slow: light foot strike (run trying',
  'Cai naturalmente conforme o pace melhora. Acompanhe a tendência, não o valor isolado.':'It falls naturally as pace improves. Follow the trend, not the isolated value.',
  'Semana acima de 100% conta como 100%: correr a mais não compensa a semana que faltou.':'A week above 100% counts as 100%: running extra does not make up for the week you missed.',
  '50 min bem soltos, terreno plano. Semana de recuperação: a bike é passeio, não treino.':'50 very relaxed min, flat terrain. Recovery week: the bike is a ride, not a workout.',
  '7 km soltos + 6 acelerações de 20 s no fim. Aceleração não é tiro: é acordar a passada.':'7 relaxed km + 6 strides of 20 s at the end. A stride is not a rep: it wakes the stride up.',
  'Saia controlado. O erro clássico é sair na adrenalina do pelotão e travar a respiração.':'Start controlled. The classic mistake is going out on the adrenaline of the pack and locking your breathing.',
  'flexor do quadril e glúteo. Sem forçar até doer — a sensação é de tensão, nunca de dor.':'hip flexor and glute. Without forcing to the point of pain — it should feel like tension, never pain.',
  'km em ritmo de prova. Nos últimos 10 minutos, aumente a cadência para preparar a troca.':'km at race pace. In the last 10 minutes, raise your cadence to prepare for the transition.',
  'mesmo esforço cardíaco — é o sinal mais honesto de base aeróbica, porque não depende de':'the same cardiac effort — it is the most honest sign of aerobic base, because it does not depend on',
  'Rodagem fácil não serve: ela diz como você recupera, não a que ritmo você aguenta 42 km.':'An easy run will not do: it tells you how you recover, not the pace you can hold for 42 km.',
  'km bem devagar. O objetivo não é treinar — é manter as pernas acordadas sem gastar nada.':'very slow km. The goal is not to train — it is to keep the legs awake without spending anything.',
  'você se esforçar mais no dia do teste. A comparação é contra as três semanas anteriores.':'you pushing harder on the day of the test. The comparison is against the three previous weeks.',
  'Últimos 4 km acelerando para 5:45/km. Terminar forte ensina o corpo a poupar glicogênio.':'Last 4 km building to 5:45/km. Finishing strong teaches the body to spare glycogen.',
  'registro — mostro os quilômetros que você fez, e a barra é o volume comparado à sua maior':'record — I show the kilometres you ran, and the bar is the volume compared with your biggest',
  '. O corpo está absorvendo. Dois quilômetros é um degrau pequeno o bastante para ser seguro':'. The body is absorbing it. Two kilometres is a step small enough to be safe',
  'Isso não é falta de disciplina, é o erro mais comum de quem corre há anos: o corpo já sabe':'That is not a lack of discipline, it is the most common mistake of people who have run for years: the body already knows',
  'O melhor plano é o que cabe na sua semana. Marque só os dias em que você realmente treina.':'The best plan is the one that fits your week. Tick only the days you actually train.',
  'corpo já está acostumado. Subir demais de uma vez é a receita clássica da lesão — e aos 64':'body is already used to. Going up too much at once is the classic recipe for injury — and at 64',
  'Esta é a parte que mais muda o plano. Todo ritmo e todo volume que o app calcula sai daqui.':'This is the part that changes the plan most. Every pace and every volume the app calculates comes from here.',
  'inteiro — e a sua ultra de 65 km aparecia como se fosse preparo atual, o que não é verdade:':'whole — and your 65 km ultra looked like current fitness, which it is not:',
  'minutos subindo devagar: comece caminhando rápido por 3 minutos e passe a trote muito leve.':'minutes building slowly: start with 3 minutes of brisk walking and move to a very easy jog.',
  'Não achei as colunas de data e distância. Confira se é o CSV de Garmin Connect › Atividades.':'I could not find the date and distance columns. Check that it is the CSV from Garmin Connect › Activities.',
  'O longão respeita três tetos: 1,15× o seu maior recente, o teto da fase e 32 km em absoluto.':'The long run respects three ceilings: 1.15× your recent longest, the phase ceiling and 32 km in absolute terms.',
  'Subida é musculação disfarçada de corrida: ganha força sem o impacto do intervalado no plano.':'Hills are weight training disguised as running: you gain strength without the impact of flat intervals in the plan.',
  'km contínuos. Primeiros 60% em rodagem, últimos 40% um pouco mais firmes. Treine a alimentação.':'continuous km. First 60% at easy pace, last 40% a little firmer. Practise your fuelling.',
  '5 minutos: panturrilha, posterior de coxa, quadril e tornozelo. 30 segundos em cada, sem forçar.':'5 minutes: calves, hamstrings, hips and ankles. 30 seconds each, without forcing.',
  '70 min em Z2. Aproveite que ainda está calor — em novembro esta sessão vira rolo dentro de casa.':'70 min in Z2. Make the most of the warm weather — in November this session moves to the indoor trainer.',
  'Ainda não chegou nenhuma corrida de hoje do Garmin. Se você já treinou, a sincronia roda de hora':'No run from today has arrived from Garmin yet. If you have already trained, the sync runs every',
  'Dá para comparar treino por treino, e é o que está logo abaixo. Mas aderência, carga e tendência':'You can compare workout by workout, and that is what is just below. But adherence, load and trend',
  'Use exatamente o que vai levar na prova. Este é o treino para descobrir o que o estômago aceita.':'Use exactly what you will carry on race day. This is the workout for finding out what your stomach accepts.',
  '. Acima de 1,5 é a faixa onde a lesão aparece — e aos 64 anos o tendão avisa depois, não durante.':'. Above 1.5 is the range where injury shows up — and at 64 the tendon warns you afterwards, not during.',
  '300 m soltos na água, se permitido. Senão, mobilidade de ombros em terra e 3 minutos de elástico.':'300 m relaxed in the water, if allowed. Otherwise, shoulder mobility on land and 3 minutes with a band.',
  'Passos curtos e rápidos, braços ativos, olhar 10 metros à frente. Não jogue o tronco para frente.':'Short, quick steps, active arms, eyes 10 metres ahead. Do not throw your torso forward.',
  'Semana de prova. O ganho já está no banco — agora o trabalho é chegar descansado, não mais forte.':'Race week. The gains are in the bank — now the work is to arrive rested, not stronger.',
  'Um gole a cada 15 minutos a partir do quilômetro 6. Acima de 90 minutos, um gel a cada 45 minutos.':'A sip every 15 minutes from kilometre 6. Above 90 minutes, a gel every 45 minutes.',
  'nas duas semanas. Amostra pequena demais para recalibrar com honestidade — o plano segue como está.':'over the two weeks. Too small a sample to recalibrate honestly — the plan stays as it is.',
  'Parar de vez depois de treino forte é o que causa aquela tontura — o sangue fica represado na perna.':'Stopping dead after a hard workout is what causes that dizziness — the blood pools in your legs.',
  '60 a 90 g de carboidrato por hora e um gole de líquido a cada 15 minutos. Comece antes de sentir fome.':'60 to 90 g of carbohydrate per hour and a sip of fluid every 15 minutes. Start before you feel hungry.',
  'Bike seguida de corrida, sem intervalo. Ensina as pernas a trocar de gesto — o que decide um triathlon.':'Bike straight into a run, no break. It teaches the legs to switch movement — what decides a triathlon.',
  '5 minutos, agora sim parado e segurando 30 segundos em cada: panturrilha, posterior de coxa, quadríceps,':'5 minutes, now static and holding 30 seconds each: calves, hamstrings, quads,',
  '8×100 m com 20 s de pausa. Se o ombro reclamar, corte pela metade — isto aqui é recuperação, não treino.':'8×100 m with 20 s rest. If your shoulder complains, cut it in half — this is recovery, not training.',
  'Subida é musculação disfarçada de corrida: ganha força de perna sem o impacto que o tiro no plano cobra.':'Hills are weight training disguised as running: you gain leg strength without the impact flat reps charge you.',
  'Rodagem fácil rápida demais é o erro clássico: ela cansa como treino forte e não rende como treino forte.':'An easy run that is too fast is the classic mistake: it tires you like a hard workout and pays like nothing.',
  'É daqui que sai o tamanho do plano. Um plano grande demais para quem está começando é a receita da lesão.':'This is where the size of the plan comes from. A plan too big for someone starting out is the recipe for injury.',
  'a um alvo de 180 spm — medido em atletas de elite em prova — não diz nada útil. A faixa verde é o esperado':'to a target of 180 spm — measured on elite athletes in competition — tells you nothing useful. The green band is what is expected',
  'atualizada no Hevy. Isto NAO marca o treino como feito — ele conta quando voce registrar a sessao no Hevy.':'updated in Hevy. This does NOT mark the workout as done — it counts when you log the session in Hevy.',
  '10 minutos de trote leve, 4 educativos e 2 acelerações de 30 segundos. Termine 15 minutos antes da largada.':'10 minutes of easy jog, 4 drills and 2 strides of 30 seconds. Finish 15 minutes before the start.',
  '5 minutos de trote muito leve, depois 5 minutos de mobilidade: panturrilha, posterior, quadril e tornozelo.':'5 minutes of very easy jog, then 5 minutes of mobility: calves, hamstrings, hips and ankles.',
  'O objetivo é só um — aquecer o músculo de verdade. Se você chegar ofegante ao fim disto, foi rápido demais.':'There is only one goal — to genuinely warm the muscle. If you arrive out of breath at the end of this, it was too fast.',
  'Prevenção de lesão e economia de movimento. Aos 64 anos é o treino que mais protege a continuidade do plano.':'Injury prevention and movement economy. At 64 it is the workout that most protects the continuity of the plan.',
  '2 × 20 minutos em esforço firme e constante, com 8 minutos de giro fácil entre eles. Sem sprint, sem oscilar.':'2 × 20 minutes at a firm, steady effort, with 8 minutes of easy spinning between them. No sprinting, no surging.',
  'Estímulo cardiovascular sem impacto nas articulações. Protege as pernas para os treinos de corrida da semana.':'Cardiovascular stimulus without impact on the joints. It protects the legs for the week\'s running workouts.',
  'Segure o ritmo. Saia mais devagar que o alvo mesmo que pareça fácil demais — é o erro mais comum e o mais caro.':'Hold the pace back. Start slower than target even if it feels too easy — it is the most common mistake and the most expensive.',
  'Você está fazendo menos que a média das últimas quatro semanas. Em semana de polimento é exatamente o esperado.':'You are doing less than the average of the last four weeks. In a taper week that is exactly what is expected.',
  '3 × 400 m em ritmo firme e constante, com 45 segundos de descanso. É o ritmo que você sustentaria por 30 minutos.':'3 × 400 m at a firm, steady pace, with 45 seconds rest. It is the pace you could hold for 30 minutes.',
  'Isto não é aquecimento, é técnica: postura, cadência e coordenação. Faça com atenção no gesto, não na velocidade.':'This is not a warm-up, it is technique: posture, cadence and coordination. Do it paying attention to the movement, not the speed.',
  'O dia passa a contar como descanso. Isso não some com o plano: dá para voltar atrás a qualquer momento, aqui mesmo.':'The day now counts as rest. This does not erase the plan: you can undo it at any time, right here.',
  '2 km aquecimento · 5×3 min a 4:35/km com 2 min de trote · 2 km soltando. Sobe o VO2max, que é o teto de tudo o mais.':'2 km warm-up · 5×3 min at 4:35/km with 2 min jog · 2 km cool-down. It raises VO2max, which is the ceiling for everything else.',
  '× 100 m em velocidade quase máxima, com 2 a 3 minutos de caminhada entre cada. A recuperação longa é parte do treino.':'× 100 m at near-maximal speed, with 2 to 3 minutes of walking between each. The long recovery is part of the workout.',
  'O CSV de atividades não traz mecânica de corrida. Esses dados vêm no arquivo .FIT de cada treino ou pela API do Garmin.':'The activity CSV carries no running mechanics. That data comes in each workout\'s .FIT file or through the Garmin API.',
  'Aderência alta, volume cumprido e a resposta do corpo está estável ou melhorando. É assim que se chega em 18 de outubro.':'High adherence, volume completed and the body\'s response steady or improving. This is how you arrive on October 18.',
  '× 4 minutos forte, com 4 minutos de giro leve entre as repetições. Mantenha a cadência acima de 85 rpm mesmo no esforço.':'× 4 minutes hard, with 4 minutes of easy spinning between reps. Keep the cadence above 85 rpm even in the effort.',
  '2 km aquecimento · 2×4 km a 5:27/km com 3 min de trote · 2 km soltando. Grave a sensação: é ela que você persegue em 18/10.':'2 km warm-up · 2×4 km at 5:27/km with 3 min jog · 2 km cool-down. Memorise the feeling: it is what you are chasing on 18/10.',
  '4 × 20 segundos no ritmo que pretende fazer na prova, com 60 segundos de caminhada entre elas. Termine querendo fazer mais.':'4 × 20 seconds at the pace you intend to race, with 60 seconds of walking between them. Finish wanting to do more.',
  'Aqui o alvo é o sistema nervoso e a mecânica da passada, não o fôlego. Cansado, você corre feio e o treino perde o sentido.':'Here the target is the nervous system and stride mechanics, not the lungs. Tired, you run badly and the workout loses its point.',
  'Separe hoje tudo que vai usar. Nada de estreia no dia da prova: equipamento novo é a causa mais comum de problema evitável.':'Lay out today everything you will use. No debuts on race day: new gear is the most common cause of an avoidable problem.',
  'Dia da prova. Nada de estreia: só o que você já testou nos longões. Confira o que os postos oferecem para não carregar tudo.':'Race day. No debuts: only what you have already tested on your long runs. Check what the aid stations offer so you do not carry everything.',
  '60 min na estrada, em Z2. Terreno ondulado como o de Quispamsis serve bem: subida na bike é força de perna sem nenhum impacto.':'60 min on the road, in Z2. Rolling terrain like Quispamsis works well: climbing on the bike is leg strength with no impact at all.',
  '5 minutos, tudo em movimento, sem segurar em nenhuma posição. Dez repetições de cada: balanço de perna para frente e para trás,':'5 minutes, all in movement, without holding any position. Ten reps of each: leg swings forward and back,',
  'Todos os seus treinos são de tarde e a prova larga às 7h. Faça pelo menos o longo de manhã, em jejum leve ou com o café da prova.':'All your workouts are in the afternoon and the race starts at 7 a.m. Do at least the long run in the morning, lightly fasted or with your race breakfast.',
  'Rodagem fácil é onde mora o volume. Ela não te deixa mais rápido sozinha — ela permite que os treinos fortes existam sem te quebrar.':'Easy running is where the volume lives. It does not make you faster on its own — it lets the hard workouts exist without breaking you.',
  'Trechos curtos e fortes sobem o VO₂max, que é o teto de tudo. Aos 64 é o que mais se perde com a idade e o que mais responde ao treino.':'Short, hard stretches raise VO₂max, which is the ceiling for everything. At 64 it is what you lose most with age and what responds most to training.',
  'segundos subindo forte numa ladeira de inclinação média. Volte trotando ou caminhando, sem pressa — a recuperação precisa ser completa.':'seconds climbing hard on a moderate hill. Come back jogging or walking, no rush — the recovery has to be complete.',
  'Limiar é o ritmo que você aguentaria por uma hora em prova. Treinar aqui empurra para cima o teto do que você sustenta sem acumular lactato.':'Threshold is the pace you could hold for an hour in a race. Training here pushes up the ceiling of what you can sustain without accumulating lactate.',
  'O longo é o treino da maratona. Ele ensina o corpo a usar gordura como combustível e endurece tendão e osso para as quatro horas de impacto.':'The long run is the marathon\'s workout. It teaches the body to use fat as fuel and toughens tendon and bone for the four hours of impact.',
  '× 1 minuto forte com 2 minutos de trote leve entre eles. Não olhe o relógio no bloco forte — corra pela sensação, como se fugisse de alguém.':'× 1 minute hard with 2 minutes of easy jog between them. Do not look at the watch in the hard block — run by feel, as if you were running from someone.',
  'km imediatamente após descer da bike. As pernas vão parecer estranhas nos primeiros 5 minutos. É normal, e é exatamente o que se está treinando.':'km immediately after getting off the bike. Your legs will feel strange for the first 5 minutes. That is normal, and it is exactly what you are training.',
  'Menos de 70% das sessões saíram. Ou a semana está grande demais, ou a vida está grande demais. Os dois têm conserto — o plano é que tem que ceder.':'Fewer than 70% of the sessions happened. Either the week is too big, or life is too big. Both have a fix — and it is the plan that has to give.',
  'Refeição conhecida e rica em carboidrato hoje à noite. Se dormir mal na véspera não tem problema — o sono das duas noites anteriores é o que conta.':'A familiar, carbohydrate-rich meal tonight. If you sleep badly the night before, no problem — it is the sleep of the two previous nights that counts.',
  'A 5:35/km, mais devagar que o alvo. Todo mundo sai rápido demais e todo mundo paga depois dos 30. Esses 40 segundos guardados valem 4 minutos no fim.':'At 5:35/km, slower than target. Everyone goes out too fast and everyone pays after 30 km. Those 40 seconds saved are worth 4 minutes at the end.',
  'Um gole de água a cada 15 min a partir do km 6. Um gel aos 40 min e a cada 40 min depois. Use exatamente o que vai usar em 18/10 — o estômago também treina.':'A sip of water every 15 min from km 6. A gel at 40 min and every 40 min after. Use exactly what you will use on 18/10 — the stomach trains too.',
  'km em ritmo bem controlado, no dia seguinte ao longo principal. O objetivo é correr com a perna já castigada — é o que mais se parece com o fim de uma ultra.':'km at a well-controlled pace, the day after the main long run. The goal is to run on already battered legs — it is the closest thing to the end of an ultra.',
  'Na estrada até o frio chegar. Em Saint John isso vale até o fim de outubro — depois desta prova a bike migra para o rolo, e o plano de inverno já vai considerar isso.':'On the road until the cold arrives. In Saint John that holds until the end of October — after this race the bike moves to the indoor trainer, and the winter plan will already take that into account.',
  'Até lá, mantenha base livre: corra no que estiver confortável, sem treino forte. Se quiser começar antes, é só adiantar a data da prova ou escolher um objetivo mais longo.':'Until then, keep the base easy: run at whatever feels comfortable, with no hard workouts. If you want to start sooner, just bring the race date forward or choose a longer goal.',
  'Correr encurta e enrijece flexor de quadril e panturrilha. Nadar move as mesmas articulações em amplitude total, sem peso do corpo. Aos 64 isso vale mais que um dia parado.':'Running shortens and stiffens the hip flexors and calves. Swimming moves the same joints through full range, with no body weight. At 64 that is worth more than a day off.',
  '5:27/km, o ritmo que você repetiu em todos os longos. Gel a cada 40 min, água em todos os postos. Não acelere quando se sentir bem — você vai se sentir bem, é assim que funciona.':'5:27/km, the pace you repeated on every long run. A gel every 40 min, water at every station. Do not speed up when you feel good — you will feel good, that is how it works.',
  'Correr no ritmo exato da prova ensina duas coisas: a economia do gesto naquela velocidade e, principalmente, como esse ritmo deve SENTIR. No dia 18 você não vai olhar o relógio o tempo todo.':'Running at the exact race pace teaches two things: the economy of the movement at that speed and, above all, how that pace should FEEL. On the 18th you will not be looking at your watch all the time.',
  'Depois dos 55, força duas vezes por semana é o que mais protege a continuidade do plano: previne lesão e melhora a economia de corrida. É o treino com melhor retorno que existe nessa faixa etária.':'After 55, strength twice a week is what most protects the continuity of the plan: it prevents injury and improves running economy. It is the training with the best return at that age.',
  'Bike dá estímulo aeróbico sem impacto. Aos 64, é o que permite manter volume sem somar desgaste nas articulações. Na estrada ainda ganha o trabalho de equilíbrio e a cabeça arejada, que o rolo não dá.':'The bike gives an aerobic stimulus without impact. At 64, it is what lets you keep volume without adding wear to the joints. On the road you also gain the balance work and the clear head that the indoor trainer does not give.',
  };
  for(var __k6 in EN_FONTE2) if(Object.prototype.hasOwnProperty.call(EN_FONTE2, __k6)) EN_FRASES[__k6] = EN_FONTE2[__k6];

  var EN_FONTE3 = {
  'Pronto. Seu uid, para colar na regra do Firebase:':'Done. Your uid, to paste into the Firebase rule:',
  'foram postas pelo defeito da parte 33, não por você':'were put there by the bug in part 33, not by you',
  'total':'total','km total':'km total'
  };
  for(var __k7 in EN_FONTE3) if(Object.prototype.hasOwnProperty.call(EN_FONTE3, __k7)) EN_FRASES[__k7] = EN_FONTE3[__k7];

  /* ── últimos pedaços que as regras pegavam pela metade ── */
  var EN_FIM = {
  '2 a 3 horas antes: algo leve e conhecido.':'2 to 3 hours before: something light and familiar.',
  '3 horas antes: algo leve e conhecido. Nada novo.':'3 hours before: something light and familiar. Nothing new.',
  '5 minutos caminhando e alongamento leve.':'5 minutes walking and light stretching.',
  '8 minutos girando leve.':'8 minutes spinning easy.',
  'Melhor marca dos últimos 12 meses — distância':'Best time of the last 12 months — distance',
  'Natação leve':'Easy swim',
  'blocos de 14 dias':'blocos de 14 dias',
  'janela de 14 dias':'janela de 14 dias',
  'm forte, com':'m hard, with',
  'semanas até a prova':'weeks to race day',
  'semanas de preparo':'weeks of build-up',
  'semana de preparo':'week of build-up'
  };
  for(var __k8 in EN_FIM) if(Object.prototype.hasOwnProperty.call(EN_FIM, __k8)) EN_FRASES[__k8] = EN_FIM[__k8];

  /* ── palavras sem acento que meu filtro deixou passar ── */
  var EN_SEM_ACENTO = {
  'Estado atual':'Current state','estado atual':'current state',
  'Excelente':'Excellent','excelente':'excellent','Otimo':'Great','Ótimo':'Great','Ótima':'Great',
  'Mediana semanal':'Weekly median','mediana':'median','Mediana':'Median',
  'caindo':'falling','subindo':'rising','estável':'steady','estavel':'steady',
  'Educativos':'Drills','educativos':'drills','Educativo':'Drill',
  'mais rápido':'faster','mais devagar':'slower','mais baixo':'lower','mais alto':'higher',
  'mais rápido →':'faster →','mais lento':'slower',
  'Bloco em andamento':'Block in progress','Bloco':'Block','bloco':'block',
  'Entrando…':'Signing in…','Enviando…':'Sending…','buscando…':'fetching…',
  'Carregando…':'Loading…','Disparando…':'Triggering…','Processando…':'Processing…',
  ...
  '. Aumente o intervalo na barra acima.':'. Widen the range on the bar above.',
  'dias. Aumente o intervalo na barra acima.':'days. Widen the range on the bar above.',
  'dias. Aumente o intervalo na barra acima para ver a tendência.':'days. Widen the range on the bar above to see the trend.',
  'Inscrever-se':'Register','dias restantes':'days left','toque numa linha':'tap a row',
  'para abrir o treino':'to open the workout','1 sem previsão':'1 unplanned',
  'sem previsão':'unplanned','Barra cheia: feito':'Solid bar: done',
  '+ feito:':'+ done:','média do período':'range average','o pico médio foi':'the average peak was',
  'Corrida':'Run','Natação':'Swim','Bike':'Bike','Força':'Strength','Caminhada':'Walk'
  };
  for(var __k9 in EN_SEM_ACENTO) if(Object.prototype.hasOwnProperty.call(EN_SEM_ACENTO, __k9)) EN_FRASES[__k9] = EN_SEM_ACENTO[__k9];

  /* ── frases dos gráficos de Saúde e Índices, medidas na tela cheia ── */
  var EN_GRAF = {
  'Cada barra vai do fundo ao pico de energia de uma semana. No período, o pico médio foi':'Each bar runs from the low to the peak of energy for one week. Over the range, the average peak was',
  'Cada barra é a média de uma semana. Média de':'Each bar is the average of one week. Average of',
  'de soneca nesse dia, fora da noite. Somando as duas,':'of nap on that day, outside the night. Adding the two,',
  'As duas marcas em cada barra são a faixa de referência para adultos. Média do período: profundo':'The two marks on each bar are the reference range for adults. Range average: deep',
  'A linha é a média de cada semana; a faixa clara mostra a variação entre as noites. De':'The line is the average of each week; the light band shows the variation between nights. From',
  'E a concordância das fases medidas por relógio com um exame de sono fica em torno de 40 a 50% — leia a tendência de semanas, nunca a fase de uma noite isolada.':'And the agreement between watch-measured stages and a sleep study is around 40 to 50% — read the trend over weeks, never one isolated night’s stage.',
  'Duas ressalvas honestas: essas faixas são da fisiologia do sono, não um padrão do Garmin.':'Two honest caveats: these ranges come from sleep physiology, not from a Garmin standard.',
  'média do período':'range average','Média do período':'Range average',
  'acima do esperado':'above expected','abaixo do esperado':'below expected',
  'O passo encurta sozinho.':'The stride shortens on its own.',
  'cada aterrissagem dessas é um freio.':'every one of those landings is a brake.',
  'sem mudar o ritmo.':'without changing pace.'
  };
  for(var __kA in EN_GRAF) if(Object.prototype.hasOwnProperty.call(EN_GRAF, __kA)) EN_FRASES[__kA] = EN_GRAF[__kA];

  var EN_SONO = {
  'Cada barra é a média de uma semana':'Each bar is the average of one week',
  'O pedaço no alto de cada barra é a soneca':'The piece at the top of each bar is the nap',
  'leia a tendência de semanas, nunca a fase de uma noite isolada.':'read the trend over weeks, never one isolated night’s stage.',
  'A faixa verde clara é onde a recuperação acontece.':'The light green band is where recovery happens.',
  'As porcentagens acima são só da noite, que é onde o relógio mede as fases.':'The percentages above are from the night only, which is where the watch measures the stages.',
  'Somando as duas,':'Adding the two,'
  };
  for(var __kB in EN_SONO) if(Object.prototype.hasOwnProperty.call(EN_SONO, __kB)) EN_FRASES[__kB] = EN_SONO[__kB];

  /* ── pedaços que o <b> corta no meio, medidos na tela cheia ── */
  var EN_CORTE = {
  'de sono em 24 h.':'of sleep in 24 h.',
  'de sono em 24 h':'of sleep in 24 h',
  'h por noite.':'h per night.',
  'ppm a passada está longa e o pé cai à frente do quadril':'spm the stride is long and the foot lands ahead of the hip',
  'cada aterrissagem dessas é um freio. Suba para':'every one of those landings is a brake. Build to',
  'ppm nas rodagens, sem mudar o ritmo.':'spm on easy runs, without changing pace.',
  'ppm nas rodagens, sem mudar o ritmo':'spm on easy runs, without changing pace',
  'ppm a cadência não é o problema. Olhe para o toque:':'spm cadence is not the problem. Look at your foot strike:',
  'a partir dos seus':'from your',
  'km reais das duas semanas anteriores':'actual km over the two previous weeks',
  'do previsto). O próximo bloco será composto em':'of plan). The next block will be built on',
  '% do previsto)':'% of plan)',
  'do previsto)':'of plan)',
  'Suba para':'Build to',
  'e o fundo médio':'and the average low',
  'No período, o pico médio foi':'Over the range, the average peak was',
  'O número no alto é a última leitura: pico e fundo.':'The number at the top is the last reading: peak and low.',
  'a faixa clara mostra a variação entre as noites.':'the light band shows the variation between nights.',
  'A linha é a média de cada semana;':'The line is the average of each week;',
  'Média de':'Average of','Média:':'Average:','média de':'average of'
  };
  for(var __kC in EN_CORTE) if(Object.prototype.hasOwnProperty.call(EN_CORTE, __kC)) EN_FRASES[__kC] = EN_CORTE[__kC];

  var EN_CORTE2 = {
  'dias com cochilo, média de':'days with a nap, average of',
  'cada aterrissagem dessas é um freio':'every one of those landings is a brake',
  'O próximo bloco será composto em':'The next block will be built on',
  'O passo encurta sozinho':'The stride shortens on its own',
  'Composto em':'Built on'
  };
  for(var __kD in EN_CORTE2) if(Object.prototype.hasOwnProperty.call(EN_CORTE2, __kD)) EN_FRASES[__kD] = EN_CORTE2[__kD];

  var EN_FIM2 = {
  'faltam':'in','Faltam':'In',
  '. A faixa verde clara é onde a recuperação acontece.':'. The light green band is where recovery happens.',
  'reais das duas semanas anteriores':'actual over the two previous weeks'
  };
  for(var __kE in EN_FIM2) if(Object.prototype.hasOwnProperty.call(EN_FIM2, __kE)) EN_FRASES[__kE] = EN_FIM2[__kE];
  ...
  /* ── padrões com curinga ──
     Quando o ${…} do código não está dentro de uma tag, tudo vira UM nó
     de texto: "Faixa calculada para o seu pace de 6:05/km, não para o
     padrão de elite." chega inteiro. Aí o dicionário por pedaço não
     serve, e é preciso o molde da frase com o número no meio. */
  var PADROES = [
    /* ── moldes escritos à mão: frases que chegam num nó só, com o
       número no meio, e que a geração automática não alcançou ── */
    [/^No mesmo batimento você está\s+(\S+)\s+mais rápido do que há três semanas\.\s*É exatamente o que a fase de base deveria produzir\.$/i,
      'At the same heart rate you are $1 faster than three weeks ago. It is exactly what the base phase should produce.'],
    [/^estável\s*\(([^)]*), dentro do ruído\)\s*que as\s+(\d+)\s+anteriores$/i,
      'steady ($1, within the noise) than the previous $2'],
    [/^A\s+(\d+)\s+ppm a passada está longa e o pé cai à frente do quadril — cada aterrissagem dessas é um freio\.\s*Suba para\s+(\S+)\s+ppm nas rodagens, sem mudar o ritmo\.\s*O passo encurta sozinho\.$/i,
      'At $1 spm the stride is long and the foot lands ahead of the hip — every one of those landings is a brake. Build to $2 spm on easy runs, without changing pace. The stride shortens on its own.'],
    [/^A\s+(\d+)\s+ppm a cadência não é o problema\.\s*Olhe para o toque:$/i,
      'At $1 spm cadence is not the problem. Look at your foot strike:'],
    [/^Composto em\s+(\S+)\s+a partir dos seus\s+(\S+)\s+km reais das duas semanas anteriores\s*\((\S+)% do previsto\)\.\s*O próximo bloco será composto em$/i,
      'Built on $1 from your actual $2 km over the two previous weeks ($3% of plan). The next block will be built on'],
    [/^A\s+(\d+)\s+ppm a passada está longa e o pé cai à frente do quadril$/i,
      'At $1 spm the stride is long and the foot lands ahead of the hip'],
    [/^Suba para\s+(.+?)\s+ppm nas rodagens, sem mudar o ritmo$/i,
      'Build to $1 spm on easy runs, without changing pace'],
    [/^km reais das duas semanas anteriores\s*\((.+?)% do previsto\)$/i,
      'actual km over the two previous weeks ($1% of plan)'],
    [/^Composto em\s+(\S+)\s+a partir dos seus\s+(.+?)\s+km reais das duas semanas anteriores\s*\((.+?)% do previsto\)$/i,
      'Built on $1 from your actual $2 km over the two previous weeks ($3% of plan)'],
    [/^Composto em\s+(\S+)\s+a partir dos seus$/i, 'Built on $1 from your'],
    [/^reais das duas semanas anteriores\s*\((.+?)% do previsto\)$/i,
      'actual over the two previous weeks ($1% of plan)'],
    [/^(.+?)\s+mais rápido$/i,  '$1 faster'],
    [/^(.+?)\s+mais devagar$/i, '$1 slower'],
    [/^(.+?)\s+mais baixo$/i,   '$1 lower'],
    [/^(.+?)\s+mais alto$/i,    '$1 higher'],
    [/^—\s*(.+?)\s+acima do esperado$/i, '— $1 above expected'],
    [/^—\s*(.+?)\s+abaixo do esperado$/i, '— $1 below expected'],
    [/^\+ feito:\s*(.+)$/i, '+ done: $1'],
    [/^média do período\s+(\S+)$/i, 'range average $1'],
    [/^(\d+)\s+de\s+(\d+)\s+(noites|semanas)\s+acima de 7 h somando noite e soneca$/i,
      '$1 of $2 nights above 7 h counting night and nap'],
    [/^de\s+(\d+)\s+(noites|semanas)\s+acima de 7 h somando noite e soneca$/i,
      'of $1 nights above 7 h counting night and nap'],
    [/^de\s+(\d+)\s+(noites|semanas)$/i, 'of $1 nights'],
    [/^Média de\s+(.+?)\s+h por noite$/i, 'Average of $1 h per night'],
    [/^(.+?)\s+dias com cochilo, média de\s+(.+?)\s+h$/i, '$1 days with a nap, average of $2 h'],
    [/^E a concordância das fases medidas por relógio com um exame de sono fica em torno de\s+(.+)$/i,
      'And the agreement between watch-measured stages and a sleep study is around $1'],
    [/^Mais\s+(.+?)\s+de soneca nesse dia, fora da noite$/i, 'Plus $1 of nap on that day, outside the night'],
    [/^Somando as duas,\s+(.+?)\s+de sono em 24 h$/i, 'Adding the two, $1 of sleep in 24 h'],
    [/^No período, o pico médio foi\s+(.+?)\s+e o fundo médio\s+(.+)$/i, 'Over the range, the average peak was $1 and the average low $2'],
    [/^E a concordância das fases medidas por relógio com um exame de sono fica em torno de\s+(.+?)\s*—\s*leia a tendência de semanas, nunca a fase de uma noite isolada\.$/i,
      'And the agreement between watch-measured stages and a sleep study is around $1 — read the trend over weeks, never one isolated night\u2019s stage.'],
    [/^(.+?)\s+de sono em 24 h\.\s*As porcentagens acima são só da noite, que é onde o relógio mede as fases\.$/i,
      '$1 of sleep in 24 h. The percentages above are from the night only, which is where the watch measures the stages.'],
    [/^(.+?)\s*minutos\s+em\s+ritmo\s+bem\s+leve\.\s+Comece\s+andando\s+3\s+minutos\s+se\s+as\s+pernas\s+estiverem\s+pesadas\.$/, '$1 minutes at a very easy pace. Start by walking 3 minutes if your legs feel heavy.'],
    [/^(.+?)\s*minutos\s+em\s+trote\s+muito\s+leve\s+até\s+a\s+respiração\s+normalizar\.$/, '$1 minutes of very easy jog until your breathing settles.'],
    [/^(.+?)\s*km\s+contínuos\s+em\s+ritmo\s+confortável\.\s+Você\s+deve\s+conseguir\s+falar\s+frases\s+inteiras\s+sem\s+ofegar\s+—\s+se\s+não\s+conseguir,\s+diminua\.$/, '$1 continuous km at a comfortable pace. You should be able to speak full sentences without gasping — if you cannot, ease off.'],
    [/^Últimos\s*(.+?)\s*km\s+um\s+pouco\s+mais\s+fortes,\s+sem\s+chegar\s+a\s+ficar\s+difícil\.\s+Simula\s+a\s+fadiga\s+da\s+prova\.$/, 'Last $1 km a little harder, without becoming difficult. It simulates race fatigue.'],
    [/^(.+?)\s*km\s+em\s+ritmo\s+bem\s+controlado,\s+no\s+dia\s+seguinte\s+ao\s+longo\s+principal\.\s+O\s+objetivo\s+é\s+correr\s+com\s+a\s+perna\s+já\s+castigada\s+—\s+é\s+o\s+que\s+mais\s+se\s+parece\s+com\s+o\s+fim\s+de\s+uma\s+ultra\.$/, '$1 km at a well-controlled pace, the day after the main long run. The goal is to run on already battered legs — it is the closest thing to the end of an ultra.'],
    [/^(.+?)\s*×\s+1\s+minuto\s+forte\s+com\s+2\s+minutos\s+de\s+trote\s+leve\s+entre\s+eles\.\s+Não\s+olhe\s+o\s+relógio\s+no\s+bloco\s+forte\s+—\s+corra\s+pela\s+sensação,\s+como\s+se\s+fugisse\s+de\s+alguém\.$/, '$1 × 1 minute hard with 2 minutes of easy jog between them. Do not look at the watch in the hard block — run by feel, as if you were running from someone.'],
    [/^(.+?)\s*×\s+100\s+m\s+em\s+velocidade\s+quase\s+máxima,\s+com\s+2\s+a\s+3\s+minutos\s+de\s+caminhada\s+entre\s+cada\.\s+A\s+recuperação\s+longa\s+é\s+parte\s+do\s+treino\.$/, '$1 × 100 m at near-maximal speed, with 2 to 3 minutes of walking between each. The long recovery is part of the workout.'],
    [/^(.+?)\s*km\s+em\s+rodagem\s+tranquila\.$/, '$1 km of easy running.'],
    [/^(.+?)\s*km\s+um\s+pouco\s+mais\s+forte,\s+ainda\s+confortável\.$/, '$1 km a little harder, still comfortable.'],
    [/^(.+?)\s*km\s+em\s+ritmo\s+de\s+limiar\.\s+Termine\s+com\s+a\s+sensação\s+de\s+que\s+daria\s+para\s+mais\s+um\s+quilômetro\.$/, '$1 km at threshold pace. Finish feeling you could have done one more kilometre.'],
    [/^(.+?)\s*km\s+no\s+ritmo\s+mais\s+leve\s+que\s+você\s+tem\.\s+Se\s+ficar\s+em\s+dúvida\s+se\s+está\s+leve\s+o\s+bastante,\s+diminua\s+mais\.$/, '$1 km at the easiest pace you have. If you are unsure whether it is easy enough, go easier still.'],
    [/^(.+?)\s*minutos\s+contínuos\s+em\s+ritmo\s+"confortavelmente\s+difícil"\.\s+Dá\s+para\s+falar\s+3\s+ou\s+4\s+palavras,\s+não\s+uma\s+frase\s+inteira\.$/, '$1 continuous minutes at a "comfortably hard" pace. You can say 3 or 4 words, not a full sentence.'],
    [/^(.+?)\s*minutos\s+contínuos\s+em\s+esforço\s+aeróbico\.\s+Deve\s+dar\s+para\s+conversar\s+o\s+tempo\s+todo\.$/, '$1 continuous minutes at aerobic effort. You should be able to talk the whole way.'],
    [/^(.+?)\s*×\s+4\s+minutos\s+forte,\s+com\s+4\s+minutos\s+de\s+giro\s+leve\s+entre\s+as\s+repetições\.\s+Mantenha\s+a\s+cadência\s+acima\s+de\s+85\s+rpm\s+mesmo\s+no\s+esforço\.$/, '$1 × 4 minutes hard, with 4 minutes of easy spinning between reps. Keep the cadence above 85 rpm even in the effort.'],
    [/^(.+?)\s*km\s+em\s+ritmo\s+aeróbico\s+constante\.\s+Se\s+tiver\s+subida,\s+mantenha\s+o\s+esforço\s+—\s+não\s+a\s+velocidade\.$/, '$1 km at a steady aerobic pace. If there is a climb, hold the effort — not the speed.'],
    [/^(.+?)\s*em\s+ritmo\s+tranquilo,\s+focando\s+em\s+braçada\s+longa\.\s+Conte\s+as\s+braçadas\s+por\s+piscina\s+e\s+tente\s+reduzir\s+uma\.$/, '$1 at an easy pace, focusing on a long stroke. Count your strokes per length and try to take one off.'],
    [/^(.+?)\s*×\s+100\s+m\s+em\s+ritmo\s+forte,\s+com\s+20\s+segundos\s+de\s+descanso\.\s+Todas\s+no\s+mesmo\s+tempo\s+—\s+anote\s+e\s+compare\s+na\s+próxima\s+semana\.$/, '$1 × 100 m at a hard pace, with 20 seconds rest. All at the same time — write it down and compare next week.'],
    [/^(.+?)\s*sem\s+parar,\s+em\s+ritmo\s+constante\.\s+Se\s+for\s+prova\s+em\s+águas\s+abertas,\s+levante\s+a\s+cabeça\s+para\s+se\s+orientar\s+a\s+cada\s+8\s+braçadas\.$/, '$1 without stopping, at a steady pace. If it is an open-water race, lift your head to sight every 8 strokes.'],
    [/^(.+?)\s*km\s+em\s+ritmo\s+de\s+prova\.\s+Nos\s+últimos\s+10\s+minutos,\s+aumente\s+a\s+cadência\s+para\s+preparar\s+a\s+troca\.$/, '$1 km at race pace. In the last 10 minutes, raise your cadence to prepare for the transition.'],
    [/^(.+?)\s*km\s+imediatamente\s+após\s+descer\s+da\s+bike\.\s+As\s+pernas\s+vão\s+parecer\s+estranhas\s+nos\s+primeiros\s+5\s+minutos\.\s+É\s+normal,\s+e\s+é\s+exatamente\s+o\s+que\s+se\s+está\s+treinando\.$/, '$1 km immediately after getting off the bike. Your legs will feel strange for the first 5 minutes. That is normal, and it is exactly what you are training.'],
    [/^(.+?)\s*tranquilos,\s+sem\s+série\.\s+Só\s+manter\s+o\s+contato\s+com\s+a\s+água\.$/, '$1 easy, no main set. Just keep the feel for the water.'],
    [/^(.+?)\s*km\s+bem\s+soltos,\s+carga\s+baixa\s+e\s+cadência\s+alta\.$/, '$1 km very relaxed, low load and high cadence.'],
    [/^(.+?)\s*km\s+bem\s+devagar\.\s+O\s+objetivo\s+não\s+é\s+treinar\s+—\s+é\s+manter\s+as\s+pernas\s+acordadas\s+sem\s+gastar\s+nada\.$/, '$1 very slow km. The goal is not to train — it is to keep the legs awake without spending anything.'],
    [/^(.+?)\s*·\s+copie\s+e\s+cole\s+no\s+MOTRA\s+para\s+programar\s+a\s+sessão\.$/, '$1 · copy and paste into MOTRA to program the session.'],
    [/^Faltam\s*(.+?)\.\s+Não\s+dá\s+tempo\s+de\s+um\s+ciclo\s+de\s*(.+?)\s*semanas\s+—\s+o\s+plano\s+entra\s+direto\s+na\s+fase\s+final,\s+com\s+foco\s+em\s+chegar\s+inteiro\.$/, 'In $1. There is not enough time for a cycle of $2 weeks — the plan goes straight into the final phase, focused on arriving in one piece.'],
    [/^Faltam\s*(.+?)\.\s+O\s+ideal\s+para\s*(.+?)\s*são\s*$/, 'In $1. The ideal for $2 are '],
    [/^,\s+começando\s+daqui\s+a\s*(.+?)\s*semana(.+?)\.\s+Até\s+lá,\s+mantenha\s+base\s+livre\.$/, ', starting in $1 week$2. Until then, keep the base easy.'],
    [/^(.+?)\s*·\s+a\s+duração\s+é\s+convertida\s+para\s+manter\s+o\s+estímulo\.$/, '$1 · the duration is converted to keep the stimulus.'],
    [/^Faixa\s+calculada\s+para\s+o\s+seu\s+pace\s+de\s*(.+?)\/km,\s+não\s+para\s+o\s+padrão\s+de\s+elite\.$/, 'Range calculated for your pace of $1/km, not to the elite standard.'],
    [/^(.+?),\s+indo\s+do\s+fundo\s+ao\s+pico\s+de\s+energia\.\s+Média:\s+pico\s*$/, '$1, running from the low to the peak of energy. Average: peak '],
    [/^Só\s*(.+?)\s*corrida(.+?)\s*em\s*(.+?)\s*dias\.\s+Aumente\s+o\s+intervalo\s+na\s+barra\s+acima\s+para\s+ver\s+a\s+tendência\.$/, 'Only $1 run$2 in $3 days. Widen the range on the bar above to see the trend.'],
    [/^Esta\s+comparação\s+precisa\s+de\s+8\s+corridas\s+com\s+frequência\s+cardíaca\.\s+Em\s*(.+?)\s*dias\s+há\s*(.+?)\.\s+Aumente\s+o\s+intervalo\s+na\s+barra\s+acima\.$/, 'This comparison needs 8 runs with heart rate. In $1 days there are $2. Widen the range on the bar above.'],
    [/^Só\s*(.+?)\s*corrida(.+?)\s*com\s+cadência\s+em\s*(.+?)\s*dias\.\s+Aumente\s+o\s+intervalo\s+na\s+barra\s+acima\.$/, 'Only $1 run$2 with cadence in $3 days. Widen the range on the bar above.'],
    [/^O\s+ritmo\s+que\s+você\s+sustenta\s+por\s+cerca\s+de\s+uma\s+hora\.\s+É\s+o\s+mais\s+importante\s+daqui:\s+dele\s+saem\s+todos\s+os\s+outros\.\s+Hoje\s+sua\s+rodagem\s+leve\s+está\s+saindo\s+em\s*(.+?)\/km\s+—\s+se\s+isso\s+parecer\s+rápido\s+demais,\s+aumente\s+este\s+número\.$/, 'The pace you can hold for about an hour. It is the most important one here: every other pace comes from it. Right now your easy runs are coming out at $1/km — if that feels too fast, raise this number.'],
    [/^(.+?)\s*dias\s+marcados\.\s+O\s+plano\s+distribui\s+os\s+treinos\s+só\s+nestes\.$/, '$1 days selected. The plan spreads workouts across these only.'],
    [/^(.+?)\.\s+No\s+período,\s+o\s+pico\s+médio\s+foi\s*$/, '$1. Over the range, the average peak was '],
    [/^(.+?)\s*acima\s+de\s+7\s+h\s+somando\s+noite\s+e\s+soneca\.\s*$/, '$1 above 7 h counting night and nap. '],
    [/^Só\s+este\s+treino\s+muda\s+de\s+dia\.\s+O\s+treino\s+principal\s+de\s*(.+?)\s*fica\s+onde\s+está\.$/, 'Only this workout moves day. The main workout of $1 stays where it is.'],
  ];
  ...
  var REGRAS = [
    ...
    /* rótulo de eixo de gráfico: "3 abr" → "Apr 3" */
    [/\b(\d{1,2})\s+(jan|fev|mar|abr|mai|jun|jul|ago|set|out|nov|dez)\b/gi,
      function(_, d, m){ return MES3[m.toLowerCase()] + ' ' + (+d) }],
    [/\b(jan|fev|mar|abr|mai|jun|jul|ago|set|out|nov|dez)\s+(\d{2,4})\b/gi,
      function(_, m, a){ return MES3[m.toLowerCase()] + ' ' + a }],
  ...
  var MAPA = null;
  /* Algumas frases estão escritas no código com \u00e9 no lugar do é.
     Na tela chega o acento de verdade, então cada chave entra duas
     vezes: como está escrita e com os acentos desdobrados. */
  function desdobrar(s){
  ...
  function indexar(){
    ...
    for(var k in EN_FRASES) if(Object.prototype.hasOwnProperty.call(EN_FRASES, k)){
      ...
      var d = desdobrar(k);
      if(d !== k) MAPA[d.toLowerCase()] = desdobrar(EN_FRASES[k]);
    }
  }
  ...
  var SEPS = [' · ', ' — ', ' – ', '. ', ': '];
  /* Só entram moldes que casam a frase INTEIRA (de ^ a $). Um molde
     solto traduziria um pedaço e devolveria o resto em português, sem
     passar pelas regras. */
  var ANCORADO = [];
  function prepararPadroes(){
    for(var i = 0; i < PADROES.length; i++){
      var f = PADROES[i][0].source;
      if(f.charAt(0) === '^' && f.charAt(f.length - 1) === '$') ANCORADO.push(PADROES[i]);
    }
  }
  function porPadrao(k){
    if(!ANCORADO.length) prepararPadroes();
    for(var i = 0; i < ANCORADO.length; i++){
      if(ANCORADO[i][0].test(k)) return k.replace(ANCORADO[i][0], ANCORADO[i][1]);
    }
    return null;
  }
  function temSeparador(k){
    for(var i = 0; i < SEPS.length; i++) if(k.indexOf(SEPS[i]) > 0) return true;
    return false;
  }
  function frase(k, prof){
    ...
    var d = dicionario(k);
    if(d != null) return d;
    /* Molde antes de partir só quando não há onde partir. Um molde que
       começa com curinga e atravessa um ponto final engoliria a frase
       anterior inteira e a devolveria em português. */
    var pd;
    if(!temSeparador(k)){
      pd = porPadrao(k);
    ...
    pd = porPadrao(k);          /* não deu para partir: tenta o molde inteiro */
    if(pd != null) return pd;
    return regras(k);
  }
  ...
  function traduzir(texto){
    ...
    /* Primeiro procura a frase inteira, com o "·" ou o "—" do começo:
       muitas entradas do dicionário incluem esse sinal. Só se não achar
       é que tiro o sinal e procuro o resto. */
    var pref = '', novo = frase(meio, 0);
    if(novo === meio){
      var mp = meio.match(/^([·—–]\s*)/);
      if(mp){
        var resto = meio.slice(mp[1].length);
        var t = frase(resto, 0);
        if(t !== resto){ pref = mp[1]; meio = resto; novo = t }
      }
    }
    if(novo === meio){
