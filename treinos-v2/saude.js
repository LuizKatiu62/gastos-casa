/* ══════════════════════════════════════════════════════════════════
   saude.js — gráficos da aba Saúde legíveis no celular

   O PROBLEMA
   Os quatro gráficos eram desenhados numa área de 680 × 200 e depois
   encolhidos para a largura do iPhone (~326 px). Tudo ficava a 48% do
   tamanho: a fonte de 10 px virava 4,8 px e a altura de 200 virava 96 px.

   A CORREÇÃO
   Área de 380 × 270, quase 1:1 com a tela do celular. A fonte sai em
   tamanho real, o gráfico fica mais alto, e sobra espaço para rótulo
   de eixo, valor do último ponto e faixas de referência.

   Também: menos barras. Acima de 26 leituras os dados passam a ser
   agrupados por semana (antes eram 40), porque 40 barras em 326 px
   dão 8 px cada — não se lê.

   COMO FUNCIONA
   Substitui as quatro funções de gráfico por fora, sem alterar o
   index.html. Se não gostar, é só remover a linha
   <script src="./saude.js"></script> e tudo volta ao que era.
   ══════════════════════════════════════════════════════════════════ */
(function(){
'use strict';
